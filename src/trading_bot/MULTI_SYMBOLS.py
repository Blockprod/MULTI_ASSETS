def purge_cache_and_state():
    """Purge le cache des indicateurs, les fichiers de cache, et l'état du bot avant chaque backtest."""
    import shutil

    # Purge le cache mémoire
    global indicators_cache, bot_state
    if isinstance(indicators_cache, dict):
        indicators_cache.clear()
    # Purge le cache disque
    cache_dir = getattr(config, "cache_dir", "./cache")
    if os.path.exists(cache_dir):
        for f in os.listdir(cache_dir):
            if f.endswith(".pkl") or f.endswith(".lock"):
                try:
    ...
    # Purge l'état du bot
    bot_state = {}
    # Supprime le fichier d'état si présent
    state_file = getattr(config, "state_file", "bot_state.pkl")
...
# --- Import Cython EMA (si dispo) ---
try:
    from backtest_engine import calculate_ema_fast
except ImportError:
    calculate_ema_fast = None


# --- Fallback EMA universel ---
def compute_ema(prices, period):
    import numpy as np

    arr = np.asarray(prices, dtype=np.float64)
    if calculate_ema_fast is not None:
        try:
            result = calculate_ema_fast(arr, period)
            if result is not None:
                return result
            else:
                print("[WARN] Cython EMA returned None, fallback to pandas.")
        except Exception as e:
            print(f"[WARN] Cython EMA failed: {e}, fallback to pandas.")
    # Fallback Python (pandas)
    import pandas as pd
...
def display_sell_signal_panel(
    ...
    # --- OPTIMISATION : accès direct aux valeurs fixes, jamais recalculées ---
    stop_loss_at_entry = pair_state.get(
        "stop_loss_at_entry", None
    )  # Stockée à l'ouverture
    ...
    # --- OPTIMISATION : accès au prix spot (évite recalculs et appels API inutiles) ---
    current_price = pair_state.get("ticker_spot_price", None)
    if current_price is None or current_price <= 0:
        current_price = row.get("close", None)

    # --- OPTIMISATION : accès direct à entry_price (jamais recalculé) ---
    ...
    # Prix d'activation du trailing stop : valeur fixée à l'ouverture
    ...
    # Stop loss : valeur fixée à l'ouverture tant que le trailing n'est pas activé
...
# LRU cache pour les indicateurs (max 30 entrées)
...
indicators_cache = LRUCache(maxsize=30)

import threading

# Global cache and lock for tickers
...
def place_trailing_stop_order(
    ...
    # --- PATCH: Update and save local state after trailing stop execution ---
    try:
        # Update pair_state and bot_state with new balances and trade info
        # Example: update coin balance, entry/exit price, trailing stop status
        symbol = params["symbol"]
        # Fetch updated balances from Binance
        ...
        coin_balance_obj = next(
            (
                ...
                if b["asset"]
                == symbol[: -len(pair_state.get("quote_currency", "USDC"))]
            ),
            None,
        )
        coin_balance = float(coin_balance_obj["free"]) if coin_balance_obj else 0.0
        pair_state["coin_balance"] = coin_balance
        pair_state["trailing_stop_activated"] = True
        pair_state["last_trade_result"] = result
        bot_state[symbol] = pair_state.copy()
        save_bot_state()
    except Exception as e:
        logger.error(f"[PATCH] Failed to update/save state after trailing stop: {e}")
    return result
...
def place_stop_loss_order(
    ...
    # --- PATCH: Update and save local state after stop-loss execution ---
    try:
        symbol = params["symbol"]
        # Fetch updated balances from Binance
        account_info = client.get_account()
        ...
        coin_balance = float(coin_balance_obj["free"]) if coin_balance_obj else 0.0
        pair_state["coin_balance"] = coin_balance
        pair_state["stop_loss_executed"] = True
        pair_state["last_trade_result"] = result
        bot_state[symbol] = pair_state.copy()
        save_bot_state()
    except Exception as e:
        logger.error(f"[PATCH] Failed to update/save state after stop-loss: {e}")
    return result


import os

# Ajoute dynamiquement le dossier du .pyd Cython compilé au sys.path
import sys

sys.path.insert(
    0,
    os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../build/lib.win-amd64-cpython-313")
    ),
)
...
class BinanceFinalClient(Client):
    ...
    def _get_ultra_safe_timestamp(self):
        ...
        Phase 2 Optimization: Réduit la fréquence de resynchronisation de 60s à 300s (5min)
        pour minimiser l'overhead. Resync automatique uniquement en cas d'erreur.
        """
        current_time = time.time()
        # Phase 2: Sync toutes les 5min au lieu de 60s (-80% overhead)
        if current_time - self._last_sync > 300 or self._error_count > 0:
            self._perform_ultra_robust_sync()
...
def get_trading_fees_from_api() -> Dict[str, float]:
    """
    Récupère les frais de trading réels depuis l'API Binance (niveau compte VIP).

    Returns:
        Dict avec 'maker_fee' et 'taker_fee' (en décimal, ex: 0.0004 pour 0.04%)

    Note: Utilise l'endpoint /api/v3/account pour récupérer les frais selon le niveau VIP.
          Les frais sont identiques pour toutes les paires de trading du compte.
    """
    try:
        # Récupérer les informations du compte qui incluent les frais
        account_info = client.get_account()

        # Les frais sont dans 'commissionRates' depuis l'API v3
        if "commissionRates" in account_info:
            maker_fee = float(account_info["commissionRates"].get("maker", 0.0004))
            taker_fee = float(account_info["commissionRates"].get("taker", 0.0007))
            logger.info(
                f"✅ Frais récupérés depuis API Binance: Maker={maker_fee*100:.2f}%, Taker={taker_fee*100:.2f}%"
            )
        else:
            # Fallback: utiliser les valeurs de configuration
            maker_fee = config.maker_fee
            taker_fee = config.taker_fee
            logger.warning(
                f"⚠️  Frais API non disponibles, utilisation des valeurs par défaut: Maker={maker_fee*100:.2f}%, Taker={taker_fee*100:.2f}%"
            )

        return {"maker_fee": maker_fee, "taker_fee": taker_fee}
    except Exception as e:
        logger.error(f"❌ Erreur lors de la récupération des frais API: {e}")
        # Fallback sur les valeurs de configuration
        return {"maker_fee": config.maker_fee, "taker_fee": config.taker_fee}


# Récupérer les frais réels depuis l'API Binance au démarrage
try:
    api_fees = get_trading_fees_from_api()
    # Mettre à jour la configuration avec les frais réels
    config.maker_fee = api_fees["maker_fee"]
    config.taker_fee = api_fees["taker_fee"]
except Exception as e:
    logger.warning(
        f"Impossible de récupérer les frais depuis l'API, utilisation des valeurs par défaut: {e}"
    )
...
# --- Centralisation du calcul d'indicateurs ---
def compute_indicators(df: pd.DataFrame, **params) -> pd.DataFrame:
    """
    Calcule les indicateurs en priorisant Cython si disponible, sinon fallback pandas.
    Utilise le cache pour éviter les recalculs.
    """
    cache_key = get_cache_key("indicators", df, params)
    if cache_key in indicators_cache:
        return indicators_cache[cache_key]
    if CYTHON_BACKTEST_AVAILABLE and backtest_engine is not None:
        # Appel Cython optimisé
        result = backtest_engine.calculate_indicators_cython(
            df,
            params.get("ema1_period", 12),
            params.get("ema2_period", 26),
            params.get("stoch_period", 14),
            params.get("sma_long", None),
            params.get("adx_period", None),
            params.get("trix_length", None),
            params.get("trix_signal", None),
        )
    else:
        # Fallback pandas
        result = universal_calculate_indicators(
            df,
            params.get("ema1_period", 12),
            params.get("ema2_period", 26),
            params.get("stoch_period", 14),
    ...
    indicators_cache[cache_key] = result
    return result


###############################################################
# Chargement de l'état du bot au démarrage
###############################################################
...
try:
    ...
    logger.warning(f"[INIT] Impossible de charger l'état du bot: {e}")
...
def update_cache_with_recent_data(
    cached_df: pd.DataFrame, pair_symbol: str, time_interval: str
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if CYTHON_BACKTEST_AVAILABLE and hasattr(
            ...
            try:
                ...
                df_work = df.copy()
                df_work["ema1"] = compute_ema(df_work["close"], ema1_period)
                df_work["ema2"] = compute_ema(df_work["close"], ema2_period)
                ...
                def compute_stochrsi(
                    rsi_series: pd.Series, period: int = 14
                ) -> pd.Series:
                ...
                if trix_length and trix_signal:
                    ema1 = compute_ema(df_work["close"], trix_length)
                    ema2 = compute_ema(ema1, trix_length)
                    ema3 = compute_ema(ema2, trix_length)
                    df_work["TRIX_PCT"] = pd.Series(ema3).pct_change() * 100
                    df_work["TRIX_SIGNAL"] = (
        ...
        try:
            if cache_key in indicators_cache:
                ...
                if len(cached_df) == len(df.dropna(subset=["close"])):
                    ...
                    # Pas de .copy() ici : on ne modifie pas cached_df dans la suite
                    return cached_df
        except (KeyError, AttributeError, TypeError):
            pass  # Cache corrompu ou indisponible -> recalcul

        # Copie de travail uniquement si on modifie le DataFrame
        df_work = (
            df
            if df is not None and df.columns.is_unique and df.index.is_unique
            else df.copy()
        )
...
def backtest_from_dataframe(
    ...
    try:
        ...
        # Phase 2: Variables pour tracking du régime de marché
        entry_regime = "normal"  # Régime au moment de l'entrée
        stop_loss_multiplier = 3.0  # Par défaut
        trailing_stop_multiplier = 5.0  # Par défaut
        ...
        for i in range(len(df_work)):
            ...
            if in_position:
                ...
                if trailing_stop_activated and row_close > max_price:
                    max_price = row_close
                    # Phase 2: Recalculer trailing stop avec multiplicateur adaptatif
                    current_atr = row_atr
                    # Réévaluer le multiplicateur si la volatilité a changé
                    trailing_stop_multiplier_current = (
                        get_adaptive_trailing_stop_multiplier(current_atr, row_close)
                    )
                    trailing_distance = trailing_stop_multiplier_current * atr_at_entry
                ...
                if exit_trade:
                    ...
                    # Calcul proceeds local (pour le backtest) avec Decimal pour précision
                    gross_proceeds_dec = Decimal(str(coin)) * Decimal(str(row_close))
                    # Utiliser maker_fee si limit orders activés, sinon taker_fee
                    fee_rate = (
                        config.maker_fee
                        if getattr(config, "use_limit_orders", True)
                        else config.taker_fee
                    )
                    fee_rate_dec = Decimal(str(fee_rate))
                    fee_dec = gross_proceeds_dec * fee_rate_dec
                    usd_dec = gross_proceeds_dec - fee_dec
                    ...
                    trade_profit = float(
                        Decimal(str(usd)) - Decimal(str(entry_usd_invested))
                    )
            ...
            if buy_condition and not in_position:
                ...
                if gross_coin and gross_coin > 0:
                    # Calculate entry with fees using Decimal for precision
                    # Utiliser maker_fee si limit orders activés, sinon taker_fee
                    fee_rate = (
                        config.maker_fee
                        if getattr(config, "use_limit_orders", True)
                    ...
                    # Reconversion en float pour compatibilité avec le reste du code
                    coin = float(coin_dec)

                    # Track USD invested (before fees) with Decimal precision
                    entry_usd_invested = float(Decimal(str(usd)))
                    usd = 0.0

                    # Set entry parameters
                    entry_price = optimized_price
                    # Phase 2: Logging avec multiplicateurs adaptatifs
                    logger.debug(
                        f"Position ouverte: entry={entry_price:.4f}, stop_loss={optimized_price - (stop_loss_multiplier * row_atr):.4f} (regime={entry_regime}, SL={stop_loss_multiplier}x ATR)"
                    )
                    max_price = optimized_price
                    # --- Phase 2: ATR fixe à l'entrée avec multiplicateurs adaptatifs ---
                    atr_at_entry = row_atr
                    stop_loss_at_entry = optimized_price - (
                        stop_loss_multiplier * atr_at_entry
                    )
                    trailing_activation_price_at_entry = optimized_price + (
                        trailing_stop_multiplier * atr_at_entry
                    )
                    stop_loss = stop_loss_at_entry
        ...
        try:
            final_price = (
                Decimal(str(df_work["close"].iloc[-1])) if in_position else Decimal("0")
            )
            coin_decimal = Decimal(str(coin))
            usd_decimal = Decimal(str(usd))
            final_wallet_decimal = compute_decimal_wallet_value(
                coin_decimal, final_price, usd_decimal
            )
        ...
        # Export CSV détaillé pour analyse PnL
        trades_df = pd.DataFrame(trades_history)
        try:
            from datetime import datetime

            export_filename = f"all_backtest_trades_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            trades_df.to_csv(export_filename, index=False, encoding="utf-8")
            print(f"[EXPORT] Historique des trades exporté dans {export_filename}")
...
def run_single_pair(pair_info):
    """
    Fonction globale pour exécuter un backtest sur une paire (compatible multiprocessing).
    """
    import logging

    # Silence all logs below WARNING in worker processes
    logging.getLogger().setLevel(logging.WARNING)
    try:
        # Exécuter le backtest SANS affichage, SANS print, SANS tqdm
        results = run_all_backtests(
            pair_info["backtest_pair"],
            pair_info["start_date"],
            pair_info["timeframes"],
            sizing_mode=pair_info.get("sizing_mode", "baseline"),
        )
        return pair_info["backtest_pair"], pair_info["real_pair"], results
    except Exception as e:
        # logger doit être accessible globalement
        logging.getLogger().error(f"Erreur backtest {pair_info['backtest_pair']}: {e}")
...
def run_parallel_backtests(
    ...
    # Préparer les arguments pour chaque tâche (pour compatibilité multiprocessing)
    pairs_with_args = []
    for pair in crypto_pairs:
        pair_args = pair.copy()
        pair_args["start_date"] = start_date
        pair_args["timeframes"] = timeframes
        pair_args["sizing_mode"] = sizing_mode
        pairs_with_args.append(pair_args)

    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        future_to_pair = {
            executor.submit(run_single_pair, pair): pair for pair in pairs_with_args
        }
...
def get_market_regime(atr: float, price: float) -> str:
    """
    [PHASE 2] Détecte le régime de marché basé sur la volatilité.

    Utilisé pour adapter dynamiquement:
    - Stop-loss multiplier (3x vs 4x ATR)
    - Trailing stop multiplier (5x vs 6x vs 7x ATR)
    - Capital usage (99.8% vs 99.5% vs 99.0%)
    ...
    Returns:
        "calm" (< 2%), "normal" (2-5%), ou "volatile" (> 5%)
    """
    try:
        if atr is None or atr <= 0 or price is None or price <= 0:
            return "normal"  # Fallback safe
        ...
        logger.warning(f"Erreur détection régime marché: {e}")
        return "normal"


def get_optimal_capital_usage(balance: float, atr: float, price: float) -> float:
    """
    Calcule dynamiquement le ratio optimal d'utilisation du capital selon la volatilité du marché.

    Logique:
    - Volatilité faible (ATR/Price < 2%) → Utiliser 99.8% du capital (risque faible)
    - Volatilité normale (2-5%) → Utiliser 99.5% du capital (standard)
    - Volatilité élevée (> 5%) → Utiliser 99.0% du capital (prudent)

    Args:
        balance: Solde disponible en quote currency
        atr: Average True Range (mesure de volatilité)
        price: Prix actuel de l'actif

    Returns:
        Ratio d'utilisation du capital (entre 0.990 et 0.998)
    """
    try:
        if atr is None or atr <= 0 or price is None or price <= 0:
            # Fallback sur config si données invalides
            return getattr(config, "capital_usage_ratio", 0.995)

        # Calculer la volatilité relative (ATR en % du prix)
        volatility_pct = (atr / price) * 100

        # Ajustement dynamique du ratio
        if volatility_pct < 2.0:
            # Marché calme → maximiser l'utilisation
            return 0.998
        elif volatility_pct < 5.0:
            # Volatilité normale → standard
            return 0.995
        else:
            # Haute volatilité → prudent
            return 0.990
    except Exception as e:
        logger.warning(f"Erreur calcul capital usage dynamique: {e}")
        return getattr(config, "capital_usage_ratio", 0.995)


def get_dynamic_stop_loss_multiplier(atr: float, price: float) -> float:
    """
    [PHASE 2] Calcule le multiplicateur de stop-loss adaptatif selon le régime de marché.

    Logique:
    - Marché calme (volatilité < 2%) → 3x ATR (plus serré, moins de perte potentielle)
    - Marché normal/volatile (≥ 2%) → 4x ATR (plus de marge, évite les faux stops)

    Impact attendu: Réduit les faux stops de 15-20% tout en préservant le capital.
    ...
    Returns:
        Multiplicateur ATR pour stop-loss (3.0 ou 4.0)
    """
    regime = get_market_regime(atr, price)

    if regime == "calm":
        return 3.0  # Marché calme: stop plus serré
    else:
        return 4.0  # Normal/volatile: stop plus large


def get_adaptive_trailing_stop_multiplier(atr: float, price: float) -> float:
    """
    [PHASE 2] Calcule le multiplicateur de trailing stop adaptatif selon le régime de marché.

    Logique:
    - Marché calme → 5x ATR (protège les gains rapidement)
    - Marché normal → 6x ATR (équilibre entre protection et laisser courir)
    - Marché volatile → 7x ATR (laisse respirer le trade, évite sortie prématurée)

    Impact attendu: +10-15% de PnL en optimisant les sorties selon la volatilité.
    ...
    Returns:
        Multiplicateur ATR pour trailing stop (5.0, 6.0, ou 7.0)
    """
    regime = get_market_regime(atr, price)

    if regime == "calm":
        return 5.0  # Protège gains rapidement
    elif regime == "normal":
        return 6.0  # Équilibre standard
    else:
        return 7.0  # Laisse respirer en volatilité élevée
...
class IncrementalIndicatorCache:
    """
    [PHASE 3] Cache incrémental intelligent pour les indicateurs.

    Réduit le temps de calcul des indicateurs de 50-100ms à 10-20ms en:
    - Stockant les derniers calculs (EMA, RSI, ATR, StochRSI)
    - Mettant à jour seulement le dernier point pour les nouvelles bougies
    - Invalidant le cache seulement si le prix change >0.5%

    Impact attendu: -80ms par trade + meilleure réactivité
    """
    ...
    def should_invalidate(self, pair: str, current_price: float) -> bool:
        """Vérifie si le cache doit être invalidé (prix change >0.5%)."""
        if pair not in self.last_price:
            self.last_price[pair] = current_price
            return False

        last_price = self.last_price[pair]
        if last_price <= 0:
            return True

        price_change = abs(current_price - last_price) / last_price
        should_invalidate = price_change > self.price_change_threshold
    ...
    def get_cached_ema(
        self, pair: str, period: int, current_price: float
    ) -> Optional[float]:
        """Récupère EMA du cache si disponible et valide."""
        cache_key = f"{pair}_ema_{period}"

        if cache_key in self.cache and not self.should_invalidate(pair, current_price):
            return self.cache[cache_key]

        return None

    def cache_ema(self, pair: str, period: int, ema_value: float):
        """Stocke EMA en cache."""
        cache_key = f"{pair}_ema_{period}"
        self.cache[cache_key] = ema_value

    def get_cached_rsi(self, pair: str, current_price: float) -> Optional[float]:
        """Récupère RSI du cache si disponible."""
        cache_key = f"{pair}_rsi"
    ...
    def cache_rsi(self, pair: str, rsi_value: float):
        """Stocke RSI en cache."""
        cache_key = f"{pair}_rsi"
        self.cache[cache_key] = rsi_value

    def clear_pair_cache(self, pair: str):
        """Efface le cache pour une paire spécifique."""
        keys_to_delete = [k for k in self.cache.keys() if k.startswith(pair)]
        for key in keys_to_delete:
            del self.cache[key]
        if pair in self.last_price:
            del self.last_price[pair]


def get_volatility_trend(atr_values: np.ndarray, window: int = 14) -> str:
    """
    [PHASE 3] Détecte la tendance de volatilité (croissante, stable, décroissante).

    Utilisé pour améliorer la détection de régime de marché:
    - Volatilité croissante + prix en hausse → Attention aux faux breakouts
    - Volatilité décroissante + prix stable → Bonne opportunité de sniper
    - Volatilité stable → Conditions normales

    Args:
        atr_values: Array de valeurs ATR (dernières N bougies)
        window: Nombre de bougies pour la tendance (défaut 14)
    ...
    try:
        ...
        # Seuil de 5% pour détecter une vraie tendance
        if change_pct > 0.05:
            return "increasing"
        ...
        logger.warning(f"Erreur détection tendance volatilité: {e}")
        return "stable"


def detect_market_structure(prices: np.ndarray, window: int = 20) -> str:
    """
    [PHASE 3] Détecte la structure du marché (trending vs ranging).

    Utilise l'analyse des hauts/bas pour déterminer:
    - "uptrend": Hauts et bas croissants (meilleur pour long)
    - "downtrend": Hauts et bas décroissants (éviter)
    - "ranging": Oscillations latérales (sniper friendly)

    Impact attendu: +5-10% en filtrant les faux signaux en downtrend.
    ...
    try:
        ...
        logger.warning(f"Erreur détection structure marché: {e}")
        return "ranging"


def get_decimal_rounded_price(price: float, decimals: int = 8) -> Decimal:
    """
    [PHASE 3] Arrondit un prix avec précision Decimal pour éviter les erreurs de float.
    ...
    Args:
        price: Prix en float
        decimals: Nombre de décimales (défaut 8 pour crypto)

    Returns:
        Prix exact en Decimal
    """
    try:
        # Convertir en string puis en Decimal pour éviter les erreurs de float
        price_str = f"{float(price):.{decimals}f}"
        return Decimal(price_str)
    except Exception as e:
        logger.warning(f"Erreur conversion Decimal: {e}")
...
def compute_decimal_wallet_value(
    ...
    """
    [PHASE 3] Calcule la valeur totale du portefeuille avec précision Decimal.

    Élimine tous les erreurs d'arrondi en utilisant Decimal au lieu de float.

    Args:
        coin_qty: Quantité de coin en Decimal
        current_price: Prix en Decimal
        usdc_balance: Solde USDC en Decimal

    Returns:
        Valeur totale du portefeuille en Decimal
    """
    try:
...
def optimize_order_quantity(
    symbol: str, quantity: float, price: float = None, side: str = "BUY"
) -> float:
    """
    Optimise la quantité d'un ordre pour respecter les filtres LOT_SIZE, MIN_NOTIONAL et PRICE_FILTER de Binance.
    Utilise Decimal pour une précision maximale.

    Args:
        symbol: Symbole de trading (format: COIN+QUOTE, ex: SOL+USDC)
        quantity: Quantité souhaitée
        price: Prix actuel (optionnel, pour vérifier MIN_NOTIONAL)
        side: "BUY" ou "SELL"

    Returns:
        Quantité optimisée en respectant tous les filtres API Binance
    """
    try:
        ...
        if not symbol_info:
            logger.warning(
                f"⚠️  Informations symbole non disponibles pour {symbol}, retour quantité brute"
            )
            return quantity
        ...
        # 1. LOT_SIZE: min_qty, max_qty, step_size
        if "LOT_SIZE" in filters:
            ...
            # Arrondir selon step_size (ROUND_DOWN pour respecter les limites)
            qty_dec = (qty_dec // step_size) * step_size

            # Vérifier min/max
            if qty_dec < min_qty:
                logger.warning(
                    f"⚠️  Quantité {float(qty_dec)} < minQty {float(min_qty)}, ajustement à minQty"
                )
                qty_dec = min_qty
            elif qty_dec > max_qty:
                logger.warning(
                    f"⚠️  Quantité {float(qty_dec)} > maxQty {float(max_qty)}, ajustement à maxQty"
                )
                qty_dec = max_qty

            quantity = float(qty_dec)

        # 2. MIN_NOTIONAL: valeur minimale de l'ordre (quantity * price)
        if "MIN_NOTIONAL" in filters and price:
            min_notional = Decimal(str(filters["MIN_NOTIONAL"].get("minNotional", 0)))
            notional = Decimal(str(quantity)) * Decimal(str(price))

            if notional < min_notional:
                # Augmenter la quantité pour atteindre le minimum
                required_qty = min_notional / Decimal(str(price))
                logger.warning(
                    f"⚠️  Notional {float(notional)} < minNotional {float(min_notional)}, ajustement quantité à {float(required_qty)}"
                )
                quantity = float(required_qty)

        # 3. MARKET_LOT_SIZE (si différent de LOT_SIZE)
        if "MARKET_LOT_SIZE" in filters:
            market_filter = filters["MARKET_LOT_SIZE"]
            market_min = Decimal(str(market_filter["minQty"]))
            market_max = Decimal(str(market_filter["maxQty"]))
            market_step = Decimal(str(market_filter["stepSize"]))

            qty_dec = Decimal(str(quantity))
            qty_dec = (qty_dec // market_step) * market_step
            qty_dec = max(market_min, min(market_max, qty_dec))
            quantity = float(qty_dec)

        logger.debug(f"✅ Quantité optimisée pour {symbol}: {quantity} (side={side})")
        return quantity

    except Exception as e:
        logger.error(f"❌ Erreur lors de l'optimisation de la quantité: {e}")
        return quantity
...
def _direct_limit_order(
    ...
    """
    Place un ordre LIMIT directement via l'API REST Binance.

    Args:
        symbol: Symbole de trading (format: COIN+QUOTE)
        side: "BUY" ou "SELL"
        price: Prix limite
        quantity: Quantité à trader
        time_in_force: "GTC" (Good Till Cancel), "IOC" (Immediate Or Cancel), "FOK" (Fill Or Kill)
        client_id: ID client optionnel pour idempotence
    """
    timestamp = int(time.time() * 1000)

    # Format price avec 8 décimales max
    ...
    if client_id:
        params_order.insert(-1, ("newClientOrderId", client_id))
    ...
    try:
        ...
        if response.status_code != 200:
            try:
                ...
                error_code = error_data.get("code", "UNKNOWN")
                error_msg = error_data.get("msg", "Unknown error")
                logger.error(f"[LIMIT ORDER ERROR] code={error_code}, msg={error_msg}")
                raise BinanceAPIException(response, error_code, error_msg)
            except (ValueError, KeyError):
                logger.error(f"[LIMIT ORDER ERROR] HTTP non-JSON: {response.text}")
                response.raise_for_status()

        result = response.json()
        logger.info(
            f"[LIMIT ORDER] {side} placed: {symbol} @ {price_str}, qty={quantity_str}"
        )
        return result
    except Exception as e:
...
def safe_limit_buy_with_fallback(
    symbol: str,
    current_price: float,
    quoteOrderQty: float,
    max_retries: int = 3,
    ...
    """
    Tente un ordre LIMIT POST_ONLY avec amélioration de prix, puis fallback vers MARKET si timeout.

    Cette fonction implémente une stratégie intelligente:
    1. Place un ordre LIMIT légèrement au-dessous du prix actuel (buy) pour être maker
    2. Attend jusqu'au timeout que l'ordre soit rempli
    3. Si timeout ou ordre non rempli, annule et passe en MARKET

    Avantages:
    - Économise 43% de frais (maker 0.04% vs taker 0.07%)
    - Améliore le prix d'entrée moyen
    - Garantit l'exécution via fallback MARKET
    """
    client_id = _generate_client_order_id("limit_buy")

    # Calcul du prix limite: légèrement en dessous du prix actuel pour être maker
    # On utilise 0.05% en dessous pour maximiser les chances d'être rempli rapidement
    limit_price = current_price * 0.9995

    # Calcul de la quantité basée sur le prix limite
    quantity = quoteOrderQty / limit_price

    try:
        # Tentative d'ordre LIMIT
        logger.info(
            f"[LIMIT BUY] Attempting limit order @ {limit_price:.8f} (current: {current_price:.8f})"
        )

        order = _direct_limit_order(
            symbol=symbol,
            side="BUY",
            price=limit_price,
            quantity=quantity,
        ...
        # Attente que l'ordre soit rempli ou timeout
        while time.time() - start_time < timeout_seconds:
            ...
            try:
                ...
                if status == "FILLED":
                    logger.info(
                        f"[LIMIT BUY SUCCESS] Order filled as MAKER (saved fees!)"
                    )
                    return order_status
                elif status in ["CANCELED", "REJECTED", "EXPIRED"]:
                    logger.warning(
                        f"[LIMIT BUY] Order {status}, falling back to MARKET"
                    )
                    break
            except Exception as e:
                logger.debug(f"[LIMIT BUY] Error checking order status: {e}")

        # Timeout atteint ou ordre non rempli -> annuler et passer en MARKET
        try:
            client.cancel_order(symbol=symbol, orderId=order_id)
            logger.info(f"[LIMIT BUY] Order cancelled after timeout, using MARKET")
        except Exception as cancel_exc:
            logger.debug(
                f"[LIMIT BUY] Cancel failed (maybe already filled): {cancel_exc}"
            )
            # Vérifier une dernière fois si rempli
            try:
                final_check = client.get_order(symbol=symbol, orderId=order_id)
                if final_check.get("status") == "FILLED":
                    return final_check
            except Exception:
                pass
        ...
        logger.warning(
            f"[LIMIT BUY] Failed to place limit order: {e}, falling back to MARKET"
        )

    # FALLBACK: Ordre MARKET
    logger.info(f"[FALLBACK] Using MARKET buy for {symbol}")
    return safe_market_buy(
        symbol=symbol, quoteOrderQty=quoteOrderQty, max_retries=max_retries
    )


def safe_limit_sell_with_fallback(
    symbol: str,
    current_price: float,
    quantity: float,
    max_retries: int = 3,
    ...
    Version SELL de la fonction précédente.
    Prix limite légèrement au-dessus pour être maker et obtenir de meilleurs frais.
    """
    client_id = _generate_client_order_id("limit_sell")

    # Prix limite: légèrement au-dessus du prix actuel pour être maker
    limit_price = current_price * 1.0005

    try:
        logger.info(
            f"[LIMIT SELL] Attempting limit order @ {limit_price:.8f} (current: {current_price:.8f})"
        )
        ...
        while time.time() - start_time < timeout_seconds:
            ...
            try:
                ...
                if status == "FILLED":
                    logger.info(
                        f"[LIMIT SELL SUCCESS] Order filled as MAKER (saved fees!)"
                    )
                    return order_status
                elif status in ["CANCELED", "REJECTED", "EXPIRED"]:
                    logger.warning(
                        f"[LIMIT SELL] Order {status}, falling back to MARKET"
                    )
                    break
            except Exception as e:
                logger.debug(f"[LIMIT SELL] Error checking order status: {e}")

        # Timeout -> annuler et passer en MARKET
        try:
            client.cancel_order(symbol=symbol, orderId=order_id)
            logger.info(f"[LIMIT SELL] Order cancelled after timeout, using MARKET")
        except Exception as cancel_exc:
            logger.debug(f"[LIMIT SELL] Cancel failed: {cancel_exc}")
            try:
                final_check = client.get_order(symbol=symbol, orderId=order_id)
                if final_check.get("status") == "FILLED":
                    return final_check
        ...
        logger.warning(
            f"[LIMIT SELL] Failed to place limit order: {e}, falling back to MARKET"
        )

    # FALLBACK: Ordre MARKET
    logger.info(f"[FALLBACK] Using MARKET sell for {symbol}")
    return safe_market_sell(symbol=symbol, quantity=quantity, max_retries=max_retries)
...
def execute_real_trades(
    ...
    """
    # [SAFETY GUARD] Empêcher les ordres sur des paires non autorisées
    ALLOWED_PAIRS = ["SOLUSDC"]  # Liste blanche des paires autorisées
    if real_trading_pair not in ALLOWED_PAIRS:
        logger.error(
            f"[SECURITY] Trading refusé sur {real_trading_pair} - Paire non autorisée ! "
            f"Autorisées: {ALLOWED_PAIRS}"
        )
        console.print(
            f"[bold red]⛔ SÉCURITÉ: Trading bloqué sur {real_trading_pair} (non autorisé)[/bold red]"
        )
    ...
    try:
        ...
        if pair_state.get("last_order_side") == "BUY" and coin_balance > 0:
            ...
            if stop_loss and global_current_price <= stop_loss:
                ...
                if quantity_rounded >= min_qty_dec:
                    ...
                    # Utiliser ordre LIMIT si configuré, sinon MARKET (stop-loss = urgence, donc préférer MARKET)
                    if getattr(config, "use_limit_orders", True):
                        # Pour stop-loss, timeout court car urgence
                        try:
                            safe_limit_sell_with_fallback(
                                symbol=real_trading_pair,
                                current_price=global_current_price,
                                quantity=float(quantity_rounded),
                                timeout_seconds=30,  # Timeout court pour stop-loss
                            )
                        except Exception:
                            # En cas d'échec, forcer MARKET
        ...
        if buy_condition and pair_state.get("last_order_side") != "BUY":
            ...
            # === OPTIMISATION 1 : SNIPER ENTRY (Timeframe 15min) ===
            ...
            if sniper_price != current_price:
                ...
                logger.info(
                    f"✅ [PHASE 1] Entree sniper optimisee: {improvement:.2f}% d'amelioration"
                )
                console.print(
                    Panel(
                        f"[bold cyan]🎯 OPTIMISATION SNIPER ACTIVEE[/bold cyan]\n\n"
                        ...
                        f"[dim]Utilise la timeframe 15min pour detecter le meilleur point d'entree[/dim]",
                        title="[bold cyan]⚡ PHASE 1 - SNIPER ENTRY[/bold cyan]",
                        border_style="cyan",
                        padding=(1, 2),
                        width=120,
                    )
            ...
            # === OPTIMISATION 2 : CAPITAL USAGE DYNAMIQUE ===
            # Ajuster selon la volatilité du marché pour maximiser l'utilisation du capital
            atr_value = row.get("atr", 0)
            capital_ratio = get_optimal_capital_usage(
                net_usdc, atr_value, current_price
            )
            ...
            logger.info(
                f"✅ [PHASE 1] Capital usage dynamique: {capital_ratio*100:.2f}% "
                f"(Volatilité: {volatility_pct:.2f}%)"
            )

            quote_amount = Decimal(str(net_usdc)) * Decimal(str(capital_ratio))

            quantity_to_buy = (quote_amount / Decimal(str(current_price))).quantize(
                step_size_dec, rounding=ROUND_DOWN
            )
            ...
            if min_qty_dec <= quantity_to_buy <= max_qty_dec:
                ...
                for retry_count in range(3):
                    try:
                        ...
                        # Recalculer le capital_ratio dynamique pour l'ordre
                        atr_value = row.get("atr", 0)
                        capital_ratio = get_optimal_capital_usage(
                            net_usdc, atr_value, current_price
                        )
                        quote_amount_for_order = float(
                            Decimal(str(net_usdc)) * Decimal(str(capital_ratio))
                        )

                        # === OPTIMISATION 3 : LIMIT ORDERS INTELLIGENTS ===
                        # Utiliser ordre LIMIT si configuré, sinon MARKET
                        use_limit = getattr(config, "use_limit_orders", False)

                        if use_limit:
                            timeout = getattr(config, "limit_order_timeout", 60)
                            logger.info(
                                f"✅ [PHASE 1] Tentative LIMIT order (timeout {timeout}s) "
                                f"pour économiser ~43% de frais"
                            )
                            ...
                            logger.info(
                                f"[PHASE 1] Ordre MARKET (use_limit_orders=False)"
                            )
            ...
            if quantity_rounded > 0:
                ...
                for retry_count in range(3):
                    try:
                        ...
                        if getattr(config, "use_limit_orders", True):
                            timeout = getattr(config, "limit_order_timeout", 60)
                            sell_order = safe_limit_sell_with_fallback(
                                symbol=real_trading_pair,
                                current_price=current_price,
                                quantity=float(quantity_rounded),
                                timeout_seconds=timeout,
...
def backtest_and_display_results(
    ...
    # Purge cache et état du bot avant chaque backtest
    purge_cache_and_state()
    """
...
if __name__ == "__main__":
    # ...existing code...
    # === EXPORT GLOBAL DE TOUS LES TRADES ===
    import pandas as pd

    # You must define all_results before using it
    # For example, run the parallel backtests and assign the result to all_results
    crypto_pairs = [
        {"backtest_pair": "SOLUSDT", "real_pair": "SOLUSDC"},
    ]
    start_date = (datetime.today() - timedelta(days=config.backtest_days)).strftime(
        "%d %B %Y"
    )
    all_results = run_parallel_backtests(
        crypto_pairs, start_date, timeframes, sizing_mode="baseline"
    )
    ...
    for backtest_pair, data in all_results.items():
        for result in data.get("results", []):
            ...
            if isinstance(trades_df, pd.DataFrame) and not trades_df.empty:
                ...
                trades_df["pair"] = backtest_pair
                trades_df["scenario"] = result.get("scenario", "")
                trades_df["timeframe"] = result.get("timeframe", "")
                trades_df["ema1"] = result.get("ema_periods", (None, None))[0]
                trades_df["ema2"] = result.get("ema_periods", (None, None))[1]
                all_trades.append(trades_df)
    if all_trades:
        all_trades_df = pd.concat(all_trades, ignore_index=True)
        try:
            all_trades_df.to_csv(
                "all_backtest_trades_export.csv", index=False, encoding="utf-8"
            )
            print(
                "[EXPORT] Tous les trades backtest exportés dans all_backtest_trades_export.csv"
            )
            ...
            try:
                ...
                with open(
                    "all_backtest_trades_export.meta.json", "w", encoding="utf-8"
                ) as f:
            ...
            print(
                f"[EXPORT ERROR] Impossible d'exporter les trades backtest: {export_exc}"
            )
    full_timestamp_resync()
...
--- Message automatique du Bot de Trading Crypto ---
            ...
            logger.info(
                f"[SCHEDULER] Configuration: backtest={backtest_pair}, real={data['real_pair']}, "
                f"tf={best_result['timeframe']}, scenario={best_params['scenario']}"
            )
            schedule.every(2).minutes.do(
            ...
            next_exec = "toutes les 2 minutes (720 exécutions/jour)"
            logger.info(
                f"Planification homogène 30-min activée pour {backtest_pair} -> {data['real_pair']} - Réactivité optimale"
            )
