...
def _check_pair_vs_exchange(
    ...
    exchange_name = _exchange_label(deps.client)
    try:
    ...
    try:
        ...
        logger.error(f"[RECONCILE] Impossible de récupérer le solde {exchange_name} pour {coin_symbol}: {e}")
        return None
    ...
    if has_real_balance and not local_in_position:
        ...
        try:
            deps.send_alert_fn(
                ...
                body_main=(
                    f"Position ouverte {exchange_name} ({coin_balance:.6f} {coin_symbol}) "
                    f"non enregistrée dans bot_state.\n\n"
                    f"entry_price restauré: {entry_price_restored}\n\n"
                    f"ACTION REQUISE: vérifier les stops manuellement sur {exchange_name}."
                ),
                client=deps.client,
            )
                ...
                try:
                    ...
                    if not has_stop:
                        logger.warning(
                            ...
                            "sur %s — repose automatique du SL à %.8f",
                            backtest_pair, exchange_name, sl_price,
                        )
                        ...
                        if _sl_result:
                            ...
                            logger.error(
                                ...
                                "vérifiez manuellement sur %s.", backtest_pair, exchange_name
                            )
                    else:
                        ...
                        if _existing_sl:
                            ...
                            deps.save_fn(force=True)
                            logger.info(
                                "[RECONCILE C-11] Stop-loss déjà actif sur %s pour %s ✓ "
                                "(orderId=%s, sl_exchange_placed=True persisté)",
                                exchange_name, backtest_pair, _existing_sl.get('orderId'),
                            )
                        else:
                            logger.info(
                                "[RECONCILE C-11] Stop-loss déjà actif sur %s pour %s ✓",
                                exchange_name, backtest_pair,
                            )
