...
required for Kraken LIVE backtest/WF when ``BACKTEST_ENGINE_MODE=cython_required``.
Live order execution remains Python-only and does not call this module.
...
def _compute_mtf_bullish(df_1h: pd.DataFrame, ema_fast: int, ema_slow: int) -> np.ndarray:
    ...
    """
    bullish = compute_mtf_bullish(df_1h, ema_fast, ema_slow)
    result_arr: np.ndarray = np.asarray(bullish.to_numpy(dtype=np.float64), dtype=np.float64)
    return result_arr
...
class CythonBacktestRequiredError(RuntimeError):
    """Raised when backtest/WF requires Cython but it is unavailable or failed."""


def _engine_mode(force_engine: Optional[str] = None) -> str:
    if force_engine:
        return force_engine
    return str(getattr(config, 'backtest_engine_mode', 'cython_required') or 'cython_required').lower()


def _cython_required(force_engine: Optional[str] = None) -> bool:
    return _engine_mode(force_engine) == 'cython_required'


def _cython_allowed(force_engine: Optional[str] = None) -> bool:
    mode = _engine_mode(force_engine)
    if mode in {'python', 'python_strict', 'python_audit'}:
        return False
    if mode in {'cython_required', 'cython_forced', 'cython'}:
        return True
    return not getattr(config, 'strict_strategy_parity', True)

# --- Core Backtest -----------------------------------------------------------

def backtest_from_dataframe(  # pyright: ignore[reportGeneralTypeIssues]
    df: pd.DataFrame,
    ema1_period: int,
    ema2_period: int,
    ...
    **_kwargs: Any,
    ...
    try:
        if df.empty or len(df) < 50:
            return {
                ...
                'max_drawdown': 0.0,
                'win_rate': 0.0,
                'engine_used': _engine_mode(force_engine),
            }
        ...
        if _cython_required(force_engine) and (
            not CYTHON_BACKTEST_AVAILABLE
            or backtest_engine is None
            or sizing_mode not in ('baseline', 'risk')
        ):
            raise CythonBacktestRequiredError(
                "cython_required_unavailable: "
                f"available={CYTHON_BACKTEST_AVAILABLE} sizing_mode={sizing_mode}"
            )
        if (
            CYTHON_BACKTEST_AVAILABLE
            and backtest_engine is not None
            and sizing_mode in ('baseline', 'risk')
            and _cython_allowed(force_engine)
        ):
            try:
                df_work = df.copy()
                ...
                if adx_period:
                    if 'adx' in df.columns:
                        ...
                        df_work['adx'] = ADXIndicator(
                            high=cast(pd.Series, df_work['high']),
                            low=cast(pd.Series, df_work['low']),
                            close=cast(pd.Series, df_work['close']),
                            window=adx_period,
                        ).adx()
                ...
                if _cython_required(force_engine):
                    raise CythonBacktestRequiredError(f"cython_required_unavailable: {e}") from e
                logger.warning(
        ...
        # === PYTHON FALLBACK ===
        if _cython_required(force_engine):
            raise CythonBacktestRequiredError("cython_required_unavailable: fallback refused")
        df_work = df.copy()
        ...
        # Shared regime and adaptive-volatility inputs used by live trading.
        _window_30d_py = atr_median_window(periods_per_year)
        _atr_s_py = df_work['atr']
        _atr_med_30d_py = _atr_s_py.rolling(window=_window_30d_py, min_periods=10).median()
        df_work['atr_median_30d'] = _atr_med_30d_py
        _atr_adaptive_mult_arr = np.asarray(
            [
                adaptive_atr_multiplier(
                ...
                for atr_value, atr_median in zip(_atr_s_py, _atr_med_30d_py)
            ],
            dtype=np.float64,
        )
        if getattr(config, 'mtf_filter_enabled', False) and isinstance(df_work.index, pd.DatetimeIndex):
            df_work['mtf_bullish'] = compute_mtf_bullish(
                df_work,
                int(getattr(config, 'mtf_ema_fast', 18)),
                int(getattr(config, 'mtf_ema_slow', 58)),
            )
        ...
        _scenario_name = (
            'StochRSI_TRIX' if trix_length
            else 'StochRSI_ADX' if adx_period
            else 'StochRSI_SMA' if sma_long
            else 'StochRSI'
        )
        _policy_best_params = {
        ...
        for i in range(len(df_work)):
            ...
            if in_position:
                max_price = next_max_price(row_close, max_price)
                if trailing_should_activate(
                    ...
                    trailing_stop = trailing_level(
                        max_price, atr_at_entry, config.atr_multiplier
                    )
                ...
                if _be_enabled and breakeven_should_activate(
                    row_close,
                    entry_price,
                    breakeven_triggered,
                    getattr(config, 'breakeven_trigger_pct', 0.015),
                ):
                    _be_profit = (row_close - entry_price) / entry_price
                    _be_new_stop = breakeven_level(entry_price, config.slippage_buy)
                ...
                if partial_enabled:
                    ...
                    if in_position and coin > 0 and entry_price > 0 and _partial_ok:
                        ...
                        if partial_stage(
                            ...
                            partial_taken_1,
                            partial_taken_2,
                            config.partial_threshold_1,
                            config.partial_threshold_2,
                        ) == 1:
                ...
                signal_exit, _ = evaluate_signal_exit(
                    df_work.iloc[i],
                    _policy_best_params,
                    config,
                    stoch_sell_exit=_stoch_sell_exit,
                )
                motif_sortie = classify_long_exit(
                    row_close,
                    stop_loss_at_entry,
                    trailing_stop_activated,
                    trailing_stop,
                ...
                exit_trade = motif_sortie is not None
            ...
            buy_condition, _ = evaluate_buy_signal(
                df_work.iloc[idx_signal],
                usd,
                _policy_best_params,
                config,
                stoch_buy_min=_stoch_buy_min,
                stoch_buy_max=_stoch_buy_max,
            )
    ...
    except CythonBacktestRequiredError:
        logger.error("Cython obligatoire pour backtest/WF mais indisponible", exc_info=True)
        raise
    except Exception as e:
...
def run_single_backtest_optimized(args: Tuple[Any, ...]) -> Dict[str, Any]:
    ...
    try:
        result = backtest_from_dataframe(
            ...
            stoch_sell_exit_override=scenario['params'].get('stoch_sell_exit'),
            sizing_mode=sizing_mode,
        ...
        return {
            ...
            'stoch_buy_min': scenario['params'].get('stoch_buy_min'),
            'stoch_buy_max': scenario['params'].get('stoch_buy_max'),
            'stoch_sell_exit': scenario['params'].get('stoch_sell_exit'),
            'engine_used': result.get('engine_used', 'unknown'),
        }
    except CythonBacktestRequiredError as e:
        logger.error("Backtest Cython obligatoire indisponible: %s", e)
        result = empty_result_dict(timeframe, ema1, ema2, scenario['name'])
        result['engine_error'] = 'cython_required_unavailable'
        return result
    except Exception as e:
...
def run_all_backtests(
    ...
    *,
    ...
    stoch_thresholds: Optional[Mapping[str, Any]] = None,
) -> List[Dict[str, Any]]:
    ...
    if stoch_thresholds is not None:
        _threshold_values = {
            'stoch_buy_min': stoch_thresholds.get('stoch_buy_min'),
            'stoch_buy_max': stoch_thresholds.get('stoch_buy_max'),
            'stoch_sell_exit': stoch_thresholds.get('stoch_sell_exit'),
        }
        _threshold_values = {k: v for k, v in _threshold_values.items() if v is not None}
        scenarios = [
            {'name': item['name'], 'params': {**item['params'], **_threshold_values}}
...
def run_parallel_backtests(
    ...
    *,
    prepare_base_dataframe_fn: Optional[Callable[..., Optional[pd.DataFrame]]] = None,
    strategy_snapshots: Optional[Mapping[str, Mapping[str, Any]]] = None,
) -> Dict[str, Any]:
    ...
    def run_single_pair(pair_info: Dict[str, Any]) -> Tuple[str, str, List[Any]]:
        try:
            results = run_all_backtests(
                ...
                stoch_thresholds=(strategy_snapshots or {}).get(pair_info["backtest_pair"]),
            )
