...
OOS_SHARPE_MIN = 0.15      # Kraken balanced: positive OOS edge without overfitting to high crypto Sharpe
OOS_WIN_RATE_MIN = 30.0    # Minimum OOS win rate (%) (default)
OOS_DECAY_MIN = 0.10       # Kraken balanced anti-overfit gate
OOS_MIN_TRADES = 6         # Kraken balanced minimum for 1h; 4h=7 and 1d=2 are timeframe-adjusted below
RISK_FREE_RATE = 0.04      # Annual risk-free rate (default, P2-03: surchargé par config)
DEFAULT_MIN_WF_BARS = 700
STOCH_BUY_MIN_GRID = (0.02, 0.05, 0.08, 0.10, 0.15)
STOCH_BUY_MAX_GRID = (0.70, 0.75, 0.80, 0.85)
STOCH_SELL_EXIT_GRID = (0.30, 0.40, 0.50)
...
def _timeframe_wf_diagnostic(
    ...
    *,
    ...
    required_folds: Optional[int],
) -> Dict[str, Any]:
    """Return an operator-readable WF eligibility diagnostic for one timeframe."""
    available_bars = int(len(df)) if df is not None else 0
    min_train_bars = DEFAULT_MIN_WF_BARS
    min_test_bars = 200
    effective_train_pct = initial_train_pct
    if timeframe == '1d':
        min_train_bars = min(min_train_bars, 350)
        effective_train_pct = min(effective_train_pct, 0.45)
    if available_bars > 0:
        initial_train_end = max(int(available_bars * effective_train_pct), min_train_bars)
        test_pool = max(0, available_bars - initial_train_end)
        test_window = max((test_pool // n_folds) if n_folds > 0 else 0, min_test_bars)
        required_bars_estimate = initial_train_end + n_folds * test_window
    else:
        required_bars_estimate = min_train_bars + n_folds * min_test_bars
    requested = required_folds if required_folds is not None else n_folds
    available_folds = len(folds)
    eligible = available_folds > 0 and (
        required_folds is None or available_folds >= required_folds
    )
    if eligible:
        reject_reason = None
    elif available_folds > 0:
        reject_reason = (
            f"{timeframe}: {available_folds}/{requested} folds, "
            f"besoin ~{required_bars_estimate} barres"
        )
    else:
        reject_reason = (
            f"{timeframe}: 0/{requested} folds, "
            f"{available_bars} barres disponibles, besoin ~{required_bars_estimate}"
        )
    return {
        'timeframe': timeframe,
        'available_bars': available_bars,
        'available_folds': available_folds,
        'required_folds': requested,
        'required_bars_estimate': int(required_bars_estimate),
        'eligible': eligible,
...
def compute_risk_metrics(
    ...
    if n_bars_total is not None and n_bars_total > _n_orig_bars * 5:
        ...
        if _n_sell >= 2 and (_n_orig_bars / max(_n_sell, 1)) > 20:
            ...
            _profs = np.asarray(_sell_rows['profit'], dtype=np.float64)
            _cur = float(equity[0])
            _pts = [_cur]
            for _p in _profs:
                _cur += float(_p)
    ...
    if trades_df is not None and not trades_df.empty and 'profit' in trades_df.columns:
        ...
        if not sell_trades.empty:
            profits = np.asarray(sell_trades['profit'], dtype=np.float64)
...
def split_walk_forward_folds(
    ...
    # même avec un historique 1d structurellement court sur certaines paires Kraken.
...
def validate_oos_result(sharpe: float, win_rate: float) -> bool:
    ...
    Gates
    -----
    - OOS annualized Sharpe  > config.oos_sharpe_min  (defaut Kraken: 0.15)
...
def run_walk_forward_validation(
    ...
    n_folds: int = 4,
    initial_train_pct: float = 0.40,
    required_folds: Optional[int] = None,
) -> Dict[str, Any]:
    """
    ...
    for cfg in top_configs:
        ...
        if required_folds is not None and len(folds) < required_folds:
            logger.warning(
                "[WF-STRICT] %s EMA(%s,%s) %s: %d/%d folds seulement; "
                "configuration ineligible au LIVE.",
                scenario_name, ema1, ema2, tf, len(folds), required_folds,
            )
            continue
...
def run_walk_forward_optuna(
    ...
    try:
        ...
        _fallback_res = run_walk_forward_validation(
        ...
        _fallback_res.setdefault('timeframe_diagnostics', {})
        _fallback_res.setdefault('eligible_timeframes', [])
    ...
    _EMPTY: Dict[str, Any] = {
        ...
        'eligible_timeframes': [],
        'wf_elapsed_seconds': 0.0,
        'wf_trials_completed': 0,
        'wf_timeout': False,
        'engine_used': 'unknown',
    }
    _wf_started_at = time.monotonic()
    _pair_label = progress_pair or 'UNKNOWN'
    _last_progress_at = _wf_started_at
    logger.info(
        "[WF_OPTUNA_START] %s trials=%d timeout=%ss required_folds=%s",
        _pair_label, n_trials, timeout_seconds or 0, required_folds or n_folds,
    )

    scenario_params_map = {s['name']: s['params'] for s in scenarios}
    scenario_names = [s['name'] for s in scenarios]
    tf_list = [tf for tf, df in base_dataframes.items() if df is not None and not df.empty and len(df) >= 50]
    timeframe_diagnostics: Dict[str, Dict[str, Any]] = {}
    eligible_timeframes: List[str] = []

    def _empty_result() -> Dict[str, Any]:
        result = dict(_EMPTY)
        result['timeframe_diagnostics'] = dict(timeframe_diagnostics)
        result['eligible_timeframes'] = list(eligible_timeframes)
        result['wf_elapsed_seconds'] = round(time.monotonic() - _wf_started_at, 3)
        return result

    if not tf_list or not scenario_names:
        for tf, df in base_dataframes.items():
            if df is not None and not df.empty:
                timeframe_diagnostics[tf] = _timeframe_wf_diagnostic(
                    tf, df, [], n_folds=n_folds,
                    initial_train_pct=initial_train_pct,
                    required_folds=required_folds,
                )
    ...
    for tf in tf_list:
        ...
        timeframe_diagnostics[tf] = diag
        if folds and (required_folds is None or len(folds) >= required_folds):
            folds_by_tf[tf] = folds
            eligible_timeframes.append(tf)
        elif folds:
            logger.warning(
                "[WF-STRICT] Optuna exclut %s: %d/%d folds disponibles.",
                tf, len(folds), required_folds,
            )
    ...
    def _objective(trial: 'optuna.Trial') -> float:
        ...
        stoch_buy_min = trial.suggest_categorical('stoch_buy_min', STOCH_BUY_MIN_GRID)
        stoch_buy_max = trial.suggest_categorical('stoch_buy_max', STOCH_BUY_MAX_GRID)
        stoch_sell_exit = trial.suggest_categorical('stoch_sell_exit', STOCH_SELL_EXIT_GRID)
        if stoch_buy_min >= stoch_buy_max:
            return float('-inf')
        ...
        for train_df, _oos_df in folds_by_tf[tf]:
            ...
            try:
                res = backtest_fn(
                    ...
                    sizing_mode=sizing_mode,
                    periods_per_year=ppy,
                    stoch_buy_min_override=stoch_buy_min,
                    stoch_buy_max_override=stoch_buy_max,
                    stoch_sell_exit_override=stoch_sell_exit,
                )
                if res.get('engine_used') is not None:
                    set_user_attr = getattr(trial, 'set_user_attr', None)
                    if callable(set_user_attr):
                        set_user_attr('engine_used', str(res.get('engine_used')))
                ...
                if type(_e).__name__ == 'CythonBacktestRequiredError':
                    raise
    ...
    def _progress_callback(study_obj: Any, _trial_obj: Any) -> None:
        nonlocal _last_progress_at
        completed = len(study_obj.trials)
        now = time.monotonic()
        if (
            completed == 1
            or completed >= n_trials
            or (progress_trials > 0 and completed % progress_trials == 0)
            or (progress_seconds > 0 and now - _last_progress_at >= progress_seconds)
        ):
            try:
                best_value = f"{float(study_obj.best_value):.4f}"
            except Exception:
                best_value = "N/A"
            logger.info(
                "[WF_OPTUNA_PROGRESS] %s trial=%d/%d best=%s elapsed=%.1fs",
                _pair_label, completed, n_trials, best_value, now - _wf_started_at,
            )
    ...
    try:
        ...
        try:
            optimize_sig = inspect.signature(study.optimize)
            optimize_params = optimize_sig.parameters
            accepts_kwargs = any(
                param.kind == inspect.Parameter.VAR_KEYWORD
                for param in optimize_params.values()
            )
            if not accepts_kwargs:
                optimize_kwargs = {
                    key: value
                    for key, value in optimize_kwargs.items()
                    if key in optimize_params
                }
        except (TypeError, ValueError):
            pass
        study.optimize(_objective, **optimize_kwargs)
    except Exception as exc:
        if type(exc).__name__ == 'CythonBacktestRequiredError':
            logger.error(
                "[WF_OPTUNA_DONE] %s status=CYTHON_REQUIRED_UNAVAILABLE elapsed=%.1fs",
                _pair_label, time.monotonic() - _wf_started_at,
            )
            result = _empty_result()
            result['reject_reason'] = 'cython_required_unavailable'
            result['engine_used'] = 'cython_required_unavailable'
            return result
        raise

    _study_trials = getattr(study, 'trials', None)
    _trials_completed = (
        len(_study_trials)
        if _study_trials is not None
        else (n_trials if getattr(study, 'best_trial', None) is not None else 0)
    )
    _elapsed = time.monotonic() - _wf_started_at
    if timeout_seconds and _trials_completed < n_trials and _elapsed >= max(0, timeout_seconds - 1):
        logger.warning(
            "[WF_OPTUNA_TIMEOUT] %s trials=%d/%d elapsed=%.1fs timeout=%ss",
            _pair_label, _trials_completed, n_trials, _elapsed, timeout_seconds,
        )
        result = _empty_result()
        result['wf_timeout'] = True
        result['wf_trials_completed'] = _trials_completed
        result['reject_reason'] = 'optuna_timeout'
        return result
    ...
    best_stoch_buy_min = float(best_params['stoch_buy_min'])
    best_stoch_buy_max = float(best_params['stoch_buy_max'])
    best_stoch_sell_exit = float(best_params['stoch_sell_exit'])
    ...
    for fold_idx, (train_df, test_df) in enumerate(folds_by_tf[best_tf]):
        ...
        try:
            is_res = backtest_fn(
                ...
                stoch_buy_min_override=best_stoch_buy_min,
                stoch_buy_max_override=best_stoch_buy_max,
                stoch_sell_exit_override=best_stoch_sell_exit,
            )
            oos_res = backtest_fn(
    ...
    if required_folds is not None and len(oos_sharpes) < required_folds:
        logger.warning(
            "[WF-STRICT] Audit Optuna incomplet: %d/%d folds exécutés; snapshot rejeté.",
            len(oos_sharpes), required_folds,
        )
        return _empty_result()
    ...
    _best_trial_attrs = getattr(study.best_trial, 'user_attrs', {}) or {}
    _engine_used = _best_trial_attrs.get('engine_used', 'cython')
    ...
    best_cfg: Dict[str, Any] = {
        ...
        'oos_total_trades': oos_total_trades,
        'passed_oos_gates': passed,
        'stoch_buy_min': best_stoch_buy_min,
        'stoch_buy_max': best_stoch_buy_max,
        'stoch_sell_exit': best_stoch_sell_exit,
        'wf_folds_completed': len(fold_details),
        'wf_folds_requested': n_folds,
        'method': 'optuna',
        'folds': fold_details,
        'optuna_n_trials': n_trials,
        'optuna_best_is_sharpe': round(study.best_value, 4),
        'engine_used': _engine_used,
        'wf_elapsed_seconds': round(time.monotonic() - _wf_started_at, 3),
        'wf_trials_completed': _trials_completed,
        'wf_timeout': False,
    ...
    logger.info(
        "[WF_OPTUNA_DONE] %s status=%s trials=%d/%d elapsed=%.1fs engine=%s",
        _pair_label,
        "PASSED" if passed else "FAILED",
        _trials_completed,
        n_trials,
        time.monotonic() - _wf_started_at,
        _engine_used,
    )
