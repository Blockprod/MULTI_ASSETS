...
def _exchange_label(client=None) -> str:
    broker = str(getattr(client, 'broker', '') or os.getenv('BROKER', 'KRAKEN')).upper()
...
def send_trading_alert_email(
    ...
    if add_spot_balance:
        try:
            ...
            body += f"\n\nSolde SPOT {_exchange_label(client)} global : {spot_balance_usdc:.2f} USDC"
...
def write_alert_unsent_fallback(subject: str, body: str) -> None:
    """Public wrapper for critical paths that must preserve unsent alerts."""
