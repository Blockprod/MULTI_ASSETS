...
class CorrelationData:
    ...
    def add_candle(self, pair: str, timestamp: float, close_price: float) -> None:
        ...
        if pair not in self._price_history:
            self._price_history[pair] = cast(
                pd.DataFrame,
                pd.DataFrame(columns=cast(Any, ["timestamp", "close"])),
            )
        ...
        new_row = cast(
            pd.DataFrame,
            pd.DataFrame({"timestamp": [timestamp], "close": [close_price]}),
        )
        self._price_history[pair] = cast(
            pd.DataFrame,
            pd.concat(cast(Any, [df, new_row]), ignore_index=True),
        )
        ...
        self._price_history[pair] = cast(
            ...
            self._price_history[pair][
                _series(self._price_history[pair], "timestamp") >= cutoff_timestamp
    ...
    def get_correlation(self, pair1: str, pair2: str) -> Optional[float]:
        ...
        merged = cast(pd.DataFrame, pd.merge(
            df1_sorted,
            df2_sorted,
            on="timestamp",
            how="inner",
        ...
        merged = cast(pd.DataFrame, merged[_series(merged, "timestamp") >= cutoff])

        if len(merged) < MIN_CANDLES_FOR_CORRELATION:
            return None

        # Calculer corrélation
        corr = cast(Any, _series(merged, "close_1")).corr(_series(merged, "close_2"))
        return float(corr) if not np.isnan(float(corr)) else None
