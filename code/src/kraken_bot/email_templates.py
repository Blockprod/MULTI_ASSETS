...
def _exchange_label() -> str:
    return "Kraken" if os.getenv("BROKER", "KRAKEN").upper() == "KRAKEN" else "Binance"


def _quote_from_pair(pair: str) -> str:
    normalized = "".join(ch for ch in pair.upper() if ch.isalnum())
    for quote in ("USDC", "USDT", "USD", "EUR", "GBP", "CAD", "AUD"):
        if normalized.endswith(quote):
            return quote
    return "USDC"


def _exchange_upper() -> str:
    return _exchange_label().upper()
...
def api_connection_failure_email(error: str) -> Tuple[str, str]:
    """Échec de connexion à l'API du broker courant."""
    error_short = str(error)[:150] + ('...' if len(str(error)) > 150 else '')
    return (
        f"[CRIT] ERREUR Connexion API {_exchange_label()}",
        f"""=== ECHEC DE CONNEXION API {_exchange_upper()} ===

Le bot n'a pas pu etablir une connexion avec l'API {_exchange_label()}.
...
2. Controler les cles API {_exchange_label()}
3. Verifier le statut de l'API {_exchange_label()}
...
def buy_executed_email(pair: str, qty: float, price: float,
    ...
    quote_currency = _quote_from_pair(pair)
    return (
...
Paire               : {pair}
Quantite            : {qty:.8f}
Prix d'entree       : {price:.4f} {quote_currency}
Montant investi     : {usdc_spent:.2f} {quote_currency}
Solde {quote_currency} apres    : {usdc_balance_after:.2f} {quote_currency}
Strategie           : {strategy}
{extra_details}
--- Message automatique du Bot de Trading Crypto ---"""
    )
