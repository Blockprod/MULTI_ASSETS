...
def update_cache_with_recent_data(
    ...
    try:
        ...
        df_recent = cast(pd.DataFrame, pd.DataFrame(
            cast(Any, recent_klines),
            columns=cast(Any, [
                'timestamp', 'open', 'high', 'low', 'close', 'volume',
                'close_time', 'quote_av', 'trades', 'tb_base_av', 'tb_quote_av', 'ignore',
            ]),
        ))
        df_recent = cast(pd.DataFrame, df_recent[['timestamp', 'open', 'high', 'low', 'close', 'volume']])
        ...
        df_recent = cast(pd.DataFrame, df_recent[df_recent.index > last_cache_time])

        if df_recent.empty:
            logger.debug(f"Aucune nouvelle bougie pour {pair_symbol} {time_interval}")
            return cached_df

        df_merged = cast(pd.DataFrame, pd.concat(cast(Any, [cached_df, df_recent])))
        df_merged = cast(pd.DataFrame, df_merged[~df_merged.index.duplicated(keep='last')])
        df_merged = cast(pd.DataFrame, df_merged.sort_index())
