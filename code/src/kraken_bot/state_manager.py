...
_secret_key_env = (
    os.environ.get('BOT_STATE_HMAC_SECRET')
    or os.environ.get('KRAKEN_SECRET_KEY')
)
if not _secret_key_env:
    raise EnvironmentError(
        "[P0-04] Variable d'environnement BOT_STATE_HMAC_SECRET ou KRAKEN_SECRET_KEY manquante. "
...
def _legacy_state_migration_allowed() -> bool:
    flag = os.environ.get('ALLOW_LEGACY_STATE_MIGRATION', '').lower()
    bot_mode = os.environ.get('BOT_MODE', config.bot_mode)
    return bot_mode != 'LIVE' and flag in {'1', 'true', 'yes', 'on'}


def _require_legacy_state_migration_allowed(format_name: str) -> None:
    if _legacy_state_migration_allowed():
        return
    raise StateError(
        f"Refus de charger un etat legacy/non signe ({format_name}). "
        "En LIVE seul le format JSON_V1 signe est accepte. "
        "Migration legacy autorisee uniquement hors LIVE avec "
...
_KNOWN_PAIR_KEYS: Set[str] = {
    ...
    'active_strategy', 'entry_strategy',                  # strategy snapshot v2
    'wf_fallback',  # I-3: WF validation status pour affichage panel
    'wf_status', 'wf_block_reason', 'wf_last_attempt_at', 'entries_ready',
    'runtime_phase', 'live_execution_mode', 'wf_session_status',
    'backtest_display_status',
    'wf_fallback_consecutive',  # S1: compteur cycles fallback consécutifs
    'next_live_run_at', 'next_hourly_run_at',
    'display_next_live', 'display_next_wf',
    'live_cycle_running', 'scheduler_ready',
    'system_status', 'system_status_detail',
    'broker', 'broker_symbol', 'broker_order_id', 'broker_client_order_id',
    'broker_pair_status', 'broker_pair_candidates',
    'history_status', 'effective_backtest_fee_taker', 'effective_backtest_fee_maker',
    'effective_live_fee_taker', 'effective_live_fee_maker',
    'kraken_fee_source', 'kraken_fee_api_taker', 'kraken_fee_api_maker',
    'kraken_fee_error',
    'pair_symbol', 'backtest_pair', 'real_trading_pair',
}
...
_KNOWN_GLOBAL_KEYS: Set[str] = {
    ...
    'reconcile_failed', 'reconcile_failed_reason',  # TS-P2-02
    'kraken_preflight', 'kraken_private_api_ok',
...
def load_state() -> Dict:
    ...
    try:
        ...
        if raw.startswith(json_header):
            ...
            _require_legacy_state_migration_allowed('HMAC_V1 pickle')
            ...
            if stripped and stripped[0:1] in (b'{', b'['):
                _require_legacy_state_migration_allowed('unsigned JSON')
                ...
                _require_legacy_state_migration_allowed('unsigned pickle')
...
def update_pair_state(bot_state: Dict, pair: str, **kwargs: Any) -> None:
    ...
    Args:
        bot_state: dictionnaire d'état global du bot.
        pair: clé de paire dans bot_state (ex: 'ONDOUSD').
