...
def _series_or_close(df: pd.DataFrame, column: str) -> pd.Series:
    if column in df.columns:
        return _series(df, column)
    ...
    **Important (C-12, anti-look-ahead-bias)** : ce calcul DOIT porter
    ...
    try:
        ...
        atr = AverageTrueRange(
            high=_series_or_close(df, 'high'),
            low=_series_or_close(df, 'low'),
            close=_series(df, 'close'),
            window=14,
        ).average_true_range()
        volatility = (atr / _series(df, 'close')).mean()
    ...
    *,
    ...
    try:
        ...
        cache_key = get_cache_key(
            "indicators",
            f"{_series(df, 'close').iloc[-1]}_{len(df)}_{df.index[-1]}",
            params,
        )
        ...
        df_work['close'] = _series(df_work, 'close').ffill().bfill()
        if bool(cast(Any, _series(df_work, 'close').isna().any())):
            logger.warning("Donnees 'close' entierement NaN apres nettoyage")
            return pd.DataFrame()

        # --- RSI (seulement si absent) ---
        if 'rsi' not in df_work.columns:
            df_work['rsi'] = RSIIndicator(_series(df_work, 'close'), window=14).rsi()

        # --- MACD (OPTIMISATION #7: Filtre Momentum MACD) ---
        try:
            macd_indicator = MACD(
                _series(df_work, 'close'), window_fast=12, window_slow=26, window_sign=9,
            )
        ...
        df_work['ema1'] = _series(df_work, 'close').ewm(span=ema1_period, adjust=False).mean()
        df_work['ema2'] = _series(df_work, 'close').ewm(span=ema2_period, adjust=False).mean()

        # --- Stochastic RSI ---
        if 'rsi' in df_work.columns:
            df_work['stoch_rsi'] = compute_stochrsi(_series(df_work, 'rsi'), period=stoch_period)

        # --- ATR ---
        df_work['atr'] = AverageTrueRange(
            high=_series_or_close(df_work, 'high'),
            low=_series_or_close(df_work, 'low'),
            close=_series(df_work, 'close'),
            window=config.atr_period,
        ).average_true_range()

        # --- SMA long ---
        if sma_long:
            df_work['sma_long'] = _series(df_work, 'close').rolling(window=sma_long).mean()

        # --- ADX ---
        if adx_period and len(df_work) >= adx_period + 2:
            try:
                df_work['adx'] = ADXIndicator(
                    high=_series(df_work, 'high'),
                    low=_series(df_work, 'low'),
                    close=_series(df_work, 'close'),
                    window=adx_period,
                ).adx()
            except Exception:
        ...
        if trix_length and trix_signal:
            trix_ema1 = _series(df_work, 'close').ewm(span=trix_length, adjust=False).mean()
...
def prepare_base_dataframe(
    ...
    *,
    ...
    for period in [14, 25, 26, 45, 50]:
        df[f'ema_{period}'] = _series(df, 'close').ewm(span=period, adjust=False).mean()

    # Indicateurs communs a tous les scenarios
    df['rsi'] = RSIIndicator(_series(df, 'close'), window=14).rsi()
    ...
    df['stoch_rsi'] = compute_stochrsi(_series(df, 'rsi'), period=stoch_period)

    # A-1: Volume SMA pour filtre volume relatif
    if 'volume' in df.columns:
        df['vol_sma'] = _series(df, 'volume').rolling(window=getattr(config, 'volume_sma_period', 20)).mean()
