"""
data_fetcher.py — Kraken historical data retrieval & validation.

Extracted from MULTI_SYMBOLS.py (P3-SRP).  Groups all functions that fetch
data from the Kraken API or validate raw OHLCV DataFrames.
...
def drop_unclosed_kraken_candle(
    df: pd.DataFrame,
    timeframe: str,
    *,
    now: Optional[pd.Timestamp] = None,
) -> pd.DataFrame:
    """Drop Kraken's current OHLC candle when it is not closed yet."""
    if df.empty or not isinstance(df.index, pd.DatetimeIndex):
        return df
    timeframe_seconds = _TIMEFRAME_SECONDS.get(str(timeframe))
    if not timeframe_seconds:
        return df
    current = cast(Any, now if now is not None else pd.Timestamp.now(tz='UTC'))
    current = cast(Any, pd.Timestamp(current))
    if current.tzinfo is None:
        current = current.tz_localize('UTC')
    else:
        current = current.tz_convert('UTC')
    last_open = cast(Any, pd.Timestamp(cast(Any, df.index[-1])))
    if last_open.tzinfo is None:
        last_open = last_open.tz_localize('UTC')
    else:
        last_open = last_open.tz_convert('UTC')
    if bool(cast(Any, last_open + pd.Timedelta(seconds=timeframe_seconds) > current)):
        logger.debug(
            "[KRAKEN-OHLC] Bougie courante retiree: timeframe=%s open=%s now=%s",
            timeframe,
            last_open,
            current,
        )
...
def _required_history_bars(client: Any, interval: str) -> int:
    getter = getattr(client, "get_history_required_bars", None)
    if callable(getter):
        try:
            return int(cast(Any, getter)(interval))
        except Exception as exc:
            logger.debug("[KRAKEN-HISTORY] required bars lookup failed: %s", exc)
    return 1150 if str(interval) == "1d" else 1500


def _has_deep_historical_source(client: Any, pair_symbol: str, interval: str) -> bool:
    for attr in (
        "has_deep_historical_source",
        "historical_csv_available",
        "csv_history_available",
        "has_historical_csv",
    ):
        checker = getattr(client, attr, None)
        if not callable(checker):
            continue
        try:
            if bool(checker(pair_symbol, interval)):
                return True
        except Exception as exc:
            logger.debug("[KRAKEN-HISTORY] %s lookup failed: %s", attr, exc)
    return False
    ...
    try:
        ...
        if not force_refresh:
            ...
            if cached_df is not None and not cached_df.empty:
                ...
                updated_df = drop_unclosed_kraken_candle(updated_df, time_interval)
                required_bars = _required_history_bars(client, time_interval)

                # Sauvegarder le cache mis à jour
                if (
                    _has_deep_historical_source(client, pair_symbol, time_interval)
                    and len(updated_df) < required_bars
                ):
                    logger.info(
                        "[KRAKEN-HISTORY] Cache insuffisant pour %s %s: %d/%d "
                        "bougies. Reconstruction via CSV historique.",
                        pair_symbol,
                        time_interval,
                        len(updated_df),
                        required_bars,
        ...
        try:
            ...
            if not verbose_logs:
                first_ts = cast(Any, pd.Timestamp(klines_raw[0][0], unit='ms')).strftime('%Y-%m-%d')
                last_ts = cast(Any, pd.Timestamp(klines_raw[-1][0], unit='ms')).strftime('%Y-%m-%d')
                logger.info(
        ...
        df = cast(pd.DataFrame, pd.DataFrame(cast(Any, all_klines), columns=cast(Any, [
            'timestamp', 'open', 'high', 'low', 'close', 'volume',
            'close_time', 'quote_av', 'trades', 'tb_base_av',
            'tb_quote_av', 'ignore',
        ])))
        df = cast(pd.DataFrame, df[['timestamp', 'open', 'high', 'low', 'close', 'volume']])
        ...
        df = drop_unclosed_kraken_candle(cast(pd.DataFrame, df), time_interval)
        ...
        logger.error(f"Erreur API Kraken: {e}")
        ...
        if any(kw in error_str.lower() for kw in network_keywords):
            ...
            if _check_net():
                ...
                try:
                    ...
                    if klines_raw:
                        df = cast(pd.DataFrame, pd.DataFrame(cast(Any, klines_raw), columns=cast(Any, [
...
def get_binance_trading_fees(
    ...
    if str(getattr(client, 'broker', '')).upper() == 'KRAKEN':
        try:
            fees = client.get_trade_fee(symbol=symbol)
            first = fees[0] if fees else {}
            taker_commission = float(first.get('takerCommission', default_taker))
            maker_commission = float(first.get('makerCommission', default_maker))
            if first.get('source') != 'api':
                raise RuntimeError(f"Frais Kraken non confirmes API pour {symbol}")
            logger.info(
                f"[FEES REELS] Kraken - Taker: {taker_commission*100:.4f}%, "
                f"Maker: {maker_commission*100:.4f}%"
            )
            return taker_commission, maker_commission
        except Exception as e:
            logger.critical(
                f"[FEES] Impossible de recuperer frais Kraken: {e}. "
                "Aucun fallback autorise."
            )
            raise
    try:
