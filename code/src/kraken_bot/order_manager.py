...
from email_utils import write_alert_unsent_fallback
...
from strategy_policy import (
    ...
    breakeven_level,
    breakeven_should_activate,
    classify_long_exit,
    entry_levels,
    next_max_price,
    partial_stage,
    trailing_activation_level,
    trailing_level,
...
_no_reference_sell_alert_last_sent: Dict[str, float] = {}
_no_reference_sell_alert_lock = threading.Lock()


def _exchange_label(deps: '_TradingDeps') -> str:
    broker = str(getattr(deps.client, "broker", "") or os.getenv("BROKER", "")).upper()
    return "Kraken" if broker == "KRAKEN" else "Binance"


def _write_reconcile_required_marker(deps: '_TradingDeps', reason: str) -> None:
    """Persist a marker forcing exchange reconciliation before new buys."""
    states_dir = getattr(deps.config, 'states_dir', '')
    if not states_dir:
        return
    try:
        os.makedirs(str(states_dir), exist_ok=True)
        marker_path = os.path.join(str(states_dir), "reconcile_required.json")
        payload = {
        ...
        logger.critical("[RECONCILE] Marker reconcile_required ecrit: %s", marker_path)
    except Exception as marker_err:
        logger.critical("[RECONCILE] Impossible d'ecrire reconcile_required: %s", marker_err)


def _new_kraken_client_order_id() -> str:
    """Kraken cl_ord_id: free text ASCII, max 18 chars."""
    return "kb" + format(time.time_ns(), "x")[-16:]


def _cap_to_max_qty(quantity: Decimal, max_qty_dec: Decimal) -> Decimal:
    """Apply maxQty only when the exchange exposes a positive maximum."""
    if max_qty_dec > 0 and quantity > max_qty_dec:
        return max_qty_dec
    return quantity


def _is_kraken_runtime_client(client: Any) -> bool:
    broker = str(getattr(client, "broker", "") or os.getenv("BROKER", "")).upper()
    return broker == "KRAKEN"


def _resolve_buy_capital(
    ctx: "_TradeCtx",
    deps: "_TradingDeps",
    since_timestamp_ms: Optional[int],
) -> float:
    """Return the quote budget from the latest SELL found for this pair."""
    del since_timestamp_ms  # latest SELL is intentionally independent from stale local state.
    last_sell_amount = float(deps.get_usdc_sells_fn(ctx.real_trading_pair, None) or 0.0)
    if last_sell_amount > 0:
        logger.info(
            "[KRAKEN-CAPITAL] Budget achat %s = derniere vente historique: %.2f %s",
            ctx.real_trading_pair,
            last_sell_amount,
            ctx.quote_currency,
        )
        return last_sell_amount
    return 0.0


def _alert_buy_blocked_no_reference_sell(
    ctx: "_TradeCtx",
    deps: "_TradingDeps",
) -> None:
    """Notify once per cooldown when a valid BUY signal has no SELL budget anchor."""
    cooldown = float(getattr(deps.config, 'email_cooldown_seconds', 300) or 300)
    now = time.time()
    key = ctx.backtest_pair or ctx.real_trading_pair
    with _no_reference_sell_alert_lock:
        if now - _no_reference_sell_alert_last_sent.get(key, 0.0) < cooldown:
            return
        _no_reference_sell_alert_last_sent[key] = now

    subject = f"[KRAKEN] Achat bloque {ctx.backtest_pair} - aucune vente de reference"
    body = (
        "Un signal BUY valide a ete detecte, mais aucun ordre d'achat n'a ete envoye.\n\n"
        f"Paire: {ctx.backtest_pair} ({ctx.real_trading_pair})\n"
        f"Actif: {ctx.coin_symbol}\n"
        f"Devise de cotation: {ctx.quote_currency}\n"
        f"Solde {ctx.quote_currency} disponible: {ctx.usdc_balance:.2f} {ctx.quote_currency}\n\n"
        "Cause: aucune transaction SELL historique propre a cette paire n'a ete trouvee.\n"
        "Regle appliquee: le montant du prochain BUY doit provenir de la derniere vente "
        "historique de la meme paire.\n\n"
        "Action du bot: achat bloque, aucune exposition creee."
    )
    try:
        sent = deps.send_alert_fn(subject=subject, body_main=body, client=deps.client)
        if sent is False:
            write_alert_unsent_fallback(subject, body)
    except Exception as exc:
        write_alert_unsent_fallback(subject, body)
        logger.warning("[BUY BLOCKED CAPITAL] Email alerte impossible pour %s: %s", key, exc)
...
class _TradingDeps:
    ...
    get_usdc_sells_fn: Callable            # latest pair SELL quote amount for next BUY
...
def _force_save(deps: '_TradingDeps') -> None:
    """Persist critical live-money state immediately."""
    deps.save_fn(force=True)


def _activate_emergency_halt(
    ctx: '_TradeCtx',
    deps: '_TradingDeps',
    reason: str,
    *,
    ...
    """Block new buys after an unprotected-position condition."""
    timestamp = datetime.now(timezone.utc).isoformat()
    message = f"{reason} | pair={ctx.real_trading_pair} | ts={timestamp}"
    if details:
        message = f"{message} | {details}"
    logger.critical("[EMERGENCY HALT] %s", message)
    with deps.bot_state_lock:
        set_emergency_halt(deps.bot_state, message)
    _write_reconcile_required_marker(deps, message)
    try:
        _force_save(deps)
    except Exception as save_err:
        logger.critical("[EMERGENCY HALT] Persistence failed: %s", save_err)
    try:
        deps.send_alert_fn(
            subject=f"[EMERGENCY HALT] {ctx.real_trading_pair}",
            body_main=(
                f"ALERTE CRITIQUE: {reason}\n\n"
                f"Paire: {ctx.real_trading_pair}\n"
                f"Details: {details or 'N/A'}\n"
                f"Timestamp UTC: {timestamp}\n\n"
                f"Les nouveaux achats sont bloques. Verifier la position et les ordres {_exchange_label(deps)}."
            ),
            client=deps.client,
        )
    except Exception as alert_err:
        logger.warning("[EMERGENCY HALT] Email impossible: %s", alert_err)


def _restore_exchange_sl(ctx: '_TradeCtx', deps: '_TradingDeps', label: str) -> bool:
    ...
    """
    ps = ctx.pair_state
    if ps.get('sl_exchange_placed') is True:
        return True
    ...
    if not _sl_price or ctx.coin_balance <= float(ctx.min_qty_dec):
        details = f"label={label}, stop={_sl_price}, balance={ctx.coin_balance}"
        logger.warning("[%s] Impossible de restaurer SL: %s", label, details)
        _activate_emergency_halt(ctx, deps, "Impossible de restaurer le stop-loss exchange", details=details)
        return False
    ...
    try:
        ...
        _activate_emergency_halt(
            ctx,
            deps,
            "Echec restauration stop-loss exchange",
            details=f"label={label}, error={_sl_err}",
        )
        return False
        logger.error("[%s] CRITIQUE: Échec restauration SL après vente bloquée: %s", label, _sl_err)


def _cancel_exchange_sl(ctx: '_TradeCtx', deps: '_TradingDeps') -> bool:
        ...
        try:
            deps.send_alert_fn(
                ...
                body_main=(
                    ...
                    f"Vérifier les ordres ouverts sur {_exchange_label(deps)}."
                ),
                client=deps.client,
            )
...
def _sync_entry_state(ctx: '_TradeCtx', last_side: Optional[str], deps: '_TradingDeps') -> None:
    ...
    if atr_value is not None and price > 0:
        ...
        if ps.get('stop_loss_at_entry') is None:
            ps['stop_loss_at_entry'] = _levels.stop_loss
            ps['stop_loss'] = ps['stop_loss_at_entry']
        if ps.get('trailing_activation_price_at_entry') is None:
            ps['trailing_activation_price_at_entry'] = _levels.trailing_activation
...
def _update_trailing_stop(ctx: '_TradeCtx', deps: '_TradingDeps') -> None:
    ...
    if trailing_activation_price is None and entry_price and atr_at_entry:
        trailing_activation_price = trailing_activation_level(
            entry_price, atr_at_entry, deps.config.atr_multiplier
        )
    ...
    # Protection : si trailing_activation_price est stale (calculé avec un ancien multiplier)
        ...
        _expected = trailing_activation_level(
            entry_price, atr_at_entry, deps.config.atr_multiplier
        )
    ...
    if ctx.current_price is not None:
        max_price = next_max_price(ctx.current_price, max_price)

    # === ACTIVATION DU TRAILING (quand prix >= entry + atr_multiplier×ATR) ===
    if not trailing_activated and trailing_activation_price:
        if ctx.current_price is not None and trailing_should_activate(
            ctx.current_price, trailing_activation_price, trailing_activated
        ):
            trailing_activated = True
            logger.info(f"[TRAILING] ⚡ ACTIVÉ à {ctx.current_price:.4f} (seuil: {trailing_activation_price:.4f})")
            # Initialiser le trailing stop
            if atr_at_entry is not None and float(atr_at_entry) > 0 and max_price is not None:
                trailing_stop_val = trailing_level(
                    max_price, float(atr_at_entry), deps.config.atr_multiplier
                )
    ...
    if trailing_activated and atr_at_entry is not None and max_price is not None:
        new_trailing = trailing_level(
            max_price, atr_at_entry, deps.config.atr_multiplier
        )
    ...
    _be_enabled = getattr(deps.config, 'breakeven_enabled', True)
    if _be_enabled and not ps.get('breakeven_triggered', False) and entry_price and entry_price > 0:
        if ctx.current_price is not None and breakeven_should_activate(
            ctx.current_price,
            entry_price,
            bool(ps.get('breakeven_triggered', False)),
            getattr(deps.config, 'breakeven_trigger_pct', 0.015),
        ):
            _be_profit = (ctx.current_price - entry_price) / entry_price
            if _be_profit >= getattr(deps.config, 'breakeven_trigger_pct', 0.015):
                _be_new_stop = breakeven_level(entry_price, deps.config.slippage_buy)
                _current_sl = ps.get('stop_loss') or 0
                if _be_new_stop > _current_sl:
                    _be_old_stop = ps.get('stop_loss')
                    ps['stop_loss'] = _be_new_stop
                    logger.info(
                        "[B-3 BREAKEVEN] Stop remonté au prix d'entrée + slippage : %g "
                        "(profit %.2f%% >= seuil %.1f%%)",
                        _be_new_stop, _be_profit * 100,
                        getattr(deps.config, 'breakeven_trigger_pct', 0.015) * 100,
                    )
...
def _execute_partial_sells(ctx: '_TradeCtx', deps: '_TradingDeps') -> None:
    ...
    if partial_stage(
        profit_pct,
        bool(ps.get('partial_taken_1', False)),
        bool(ps.get('partial_taken_2', False)),
        deps.config.partial_threshold_1,
        deps.config.partial_threshold_2,
    ) == 1:
        # F-1: Annuler le SL exchange pour libérer les coins lockés avant vente partielle
        if not _cancel_exchange_sl(ctx, deps):
            return
    ...
    if ctx.coin_balance > 0 and partial_stage(
        profit_pct,
        bool(ps.get('partial_taken_1', False)),
        bool(ps.get('partial_taken_2', False)),
        deps.config.partial_threshold_1,
...
def _execute_one_partial(ctx: '_TradeCtx', deps: '_TradingDeps', *, partial_number: int, sell_pct: float, profit_pct: float) -> None:
    ...
    quantity_rounded = _cap_to_max_qty(quantity_rounded, ctx.max_qty_dec)
            ...
            if _sl_price and ctx.coin_balance > ctx.min_qty:
                try:
                    ...
                    _activate_emergency_halt(
                        ctx,
                        deps,
                        "Echec replacement stop-loss apres vente partielle",
                        details=f"label={label}, error={_sl_err}",
                    )
...
def _handle_exchange_sl_fill(
        ...
        try:
            ...
            if _sl_status == 'FILLED':
                ...
                logger.info(
                    ...
                    "vente gérée automatiquement par %s.",
                    sl_oid, _sl_status, _exchange_label(deps),
                )
    ...
    if not _sl_filled:
        # Exchange gère la vente — attendre le prochain cycle
        return True
    ...
    ps.update({
        ...
        'entry_scenario': None, 'entry_timeframe': None, 'entry_strategy': None,
    ...
    if quantity_rounded >= ctx.min_qty_dec and order_value >= ctx.min_notional:
        ...
        if stop_loss_order and stop_loss_order.get('status') == 'FILLED':
            ...
            _activate_emergency_halt(
                ctx,
                deps,
                "Stop-loss software market-sell non FILLED",
                details=f"status={_status}, quantity={qty_str}, stop={effective_stop}",
            )
            try:
                deps.send_alert_fn(
...
def _check_and_execute_stop_loss(ctx: '_TradeCtx', deps: '_TradingDeps') -> bool:
    ...
    if ctx.current_price is None:
        return False
    _exit_reason = classify_long_exit(
        ctx.current_price,
        stop_loss_fixed,
    ...
    if _exit_reason is None:
        return False
    is_trailing_stop = _exit_reason == 'TRAILING_STOP'
    effective_stop = trailing_stop if is_trailing_stop else stop_loss_fixed
        ...
        if _sl_is_stale:
            ...
            try:
                ...
                logger.error(
                    ...
                    "délégation à %s maintenue.",
                    _stale_oid, _sync_err, _exchange_label(deps),
                )
...
def _handle_dust_cleanup(ctx: '_TradeCtx', deps: '_TradingDeps') -> Optional[bool]:
    ...
    if has_dust:
        ...
        if dust_notional_value < ctx.min_notional:
            ...
            logger.debug(f"[DUST] Impossible de vendre le résidu - {_exchange_label(deps)} refuse les ordres < {ctx.min_notional:.2f} USDC")
            ...
            try:
                ...
                if dust_sell_order and dust_sell_order.get('status') == 'FILLED':
                    ...
                    logger.warning("[DUST] Vente du résidu échouée - %s refuse", _exchange_label(deps))
...
def _execute_signal_sell(ctx: '_TradeCtx', deps: '_TradingDeps') -> None:
    ...
    _row_atr = ctx.row.get('atr')
    if entry_price_for_panel and _row_atr:
        _fallback_levels = entry_levels(
            float(entry_price_for_panel),
            float(_row_atr),
            ctx.row.get('atr_median_30d'),
            atr_stop_multiplier=deps.config.atr_stop_multiplier,
            atr_multiplier=deps.config.atr_multiplier,
        )
        ps.setdefault('stop_loss_at_entry', _fallback_levels.stop_loss)
        ps.setdefault('stop_loss', _fallback_levels.stop_loss)
        ps.setdefault(
            'trailing_activation_price_at_entry',
            _fallback_levels.trailing_activation,
        )
        ps.setdefault(
            'atr_stop_multiplier_at_entry',
            _fallback_levels.atr_stop_multiplier,
        )
    ps.setdefault('atr_at_entry', ctx.row.get('atr'))
    ps.setdefault('max_price', entry_price_for_panel)

    if ctx.row.get('_data_stale'):
        logger.warning("[SELL BLOCKED STALE-DATA] %s - signal sell ignored on stale candle", ctx.backtest_pair)
        display_sell_signal_panel(
            row=ctx.row, coin_balance=ctx.coin_balance, pair_state=ps,
            sell_triggered=False, console=deps.console, coin_symbol=ctx.coin_symbol,
            sell_reason='STALE_DATA', best_params=ctx.best_params,
        )
        return

    _entry_snapshot = ps.get('entry_strategy')
    _ps_sell_exit = (
        _entry_snapshot.get('stoch_sell_exit')
        if isinstance(_entry_snapshot, dict)
        else ps.get('stoch_sell_exit')
    )
                ...
                if quantity_rounded >= ctx.min_qty_dec and notional_value >= ctx.min_notional:
                    ...
                    if sell_order and sell_order.get('status') == 'FILLED':
                        ...
                        try:
                            ...
                            _, _equity_after, _, _ = _get_coin_balance(account_info, ctx.quote_currency)
                            log_trade(
                                logs_dir=logs_dir,
                                pair=ctx.real_trading_pair,
                                side='sell',
                        ...
                        _restore_exchange_sl(ctx, deps, "SIGNAL-SELL-NON-FILLED")
                else:
                    if quantity_rounded < ctx.min_qty_dec:
                    ...
                    try:
                        deps.send_alert_fn(
                            ...
                            body_main=(
                                ...
                                f"Action requise: vérifier les ordres ouverts sur {_exchange_label(deps)}."
                            ),
                            client=deps.client,
                        )
                    except Exception as _email_err:
                    ...
                    _restore_exchange_sl(ctx, deps, "SIGNAL-SELL-BLOCKED")
        except Exception as e:
            logger.error(f"[VENTE] Erreur lors de l'exécution : {e}")

            _restore_exchange_sl(ctx, deps, "SIGNAL-SELL-EXCEPTION")
...
def _compute_buy_quantity(
    ...
    if quantity_rounded < min_qty_dec:
        quantity_rounded = quantity_decimal
    quantity_rounded = _cap_to_max_qty(quantity_rounded, max_qty_dec)
...
def _validate_buy_preconditions(
    ...
    if deps.bot_state.get('emergency_halt'):
        logger.critical(
            "[BUY BLOCKED EMERGENCY] %s — %s",
            ctx.backtest_pair,
            deps.bot_state.get('emergency_halt_reason', 'raison inconnue'),
        )
        return False

    if (
        getattr(deps.config, 'bot_mode', 'DEMO') == 'LIVE'
        and ctx.sizing_mode == 'baseline'
        and not bool(getattr(deps.config, 'allow_baseline_live', False))
    ):
        logger.critical(
            "[BUY BLOCKED RISK] %s — sizing_mode=baseline interdit en LIVE "
            "sans ALLOW_BASELINE_LIVE=true.",
            ctx.backtest_pair,
        )
        return False
    ...
    if (
        getattr(deps.config, 'bot_mode', 'DEMO') == 'LIVE'
        and ctx.sizing_mode not in {'risk', 'baseline'}
    ):
        logger.critical(
            "[BUY BLOCKED RISK] %s — sizing_mode=%s non implemente en LIVE Kraken.",
            ctx.backtest_pair,
            ctx.sizing_mode,
        )
        return False

    if (getattr(deps.config, 'bot_mode', 'DEMO') == 'LIVE'
            and not ctx.best_params.get('_entries_ready', False)):
        _wf_status = ps.get('wf_status') or 'pending_revalidation'
        _wf_reason = ps.get('wf_block_reason') or 'snapshot courant non revalidé'
        logger.warning(
            "[BUY BLOCKED WF-READY] %s — Tradable=NON status=%s reason=%s",
            ctx.backtest_pair, _wf_status, _wf_reason,
        )
        return False
    ...
    max_concurrent = int(getattr(deps.config, 'max_concurrent_long', 0) or 0)
    if max_concurrent > 0:
        open_longs = sum(
            1
            for value in deps.bot_state.values()
            if isinstance(value, Mapping) and value.get('last_order_side') == 'BUY'
        )
        if open_longs >= max_concurrent:
            logger.warning(
                "[BUY BLOCKED RISK] %s — max_concurrent_long atteint (%d/%d).",
                ctx.backtest_pair,
                open_longs,
                max_concurrent,
            )
    ...
    if ctx.row.get('_data_stale'):
        logger.warning(
            "[BUY BLOCKED STALE-DATA] %s - derniere bougie stale; achat bloque.",
            ctx.backtest_pair,
        )
        return False

    strict_mode = getattr(deps.config, 'oos_strict_mode', True)
    if strict_mode and ps.get('wf_fallback') is True:
        logger.warning(
            "[BUY BLOCKED WF] %s - wf_fallback=True en strict mode; achat bloque.",
            ctx.backtest_pair,
        )
        return False
...
def _compute_and_validate_quantity(
    ...
    _effective_atr_stop_multiplier = adaptive_atr_multiplier(
        deps.config.atr_stop_multiplier,
        atr_value,
        ctx.row.get('atr_median_30d') if hasattr(ctx.row, 'get') else None,
    )
    ...
    qty_result = _compute_buy_quantity(
        ...
        atr_stop_multiplier=_effective_atr_stop_multiplier,
    )
...
def _lookup_buy_by_client_order_id(
    ctx: '_TradeCtx',
    deps: '_TradingDeps',
    client_order_id: str,
) -> Optional[Dict[str, Any]]:
    """Recover a BUY order submitted with cl_ord_id after transport errors."""
    try:
        order = deps.client.get_order(
            symbol=ctx.real_trading_pair,
            origClientOrderId=client_order_id,
        )
    except Exception as lookup_err:
        logger.critical(
            "[BUY IDEMPOTENCY] Lookup cl_ord_id=%s impossible pour %s: %s",
            client_order_id,
            ctx.real_trading_pair,
            lookup_err,
        )
    ...
    if not order:
        return None
    if str(order.get('side', '')).upper() and str(order.get('side', '')).upper() != 'BUY':
        logger.critical(
            "[BUY IDEMPOTENCY] cl_ord_id=%s retrouve un ordre non-BUY: %s",
            client_order_id,
            order,
        )
        return None
    logger.warning(
        "[BUY IDEMPOTENCY] Ordre retrouve par cl_ord_id=%s: orderId=%s status=%s",
        client_order_id,
        order.get('orderId'),
        order.get('status'),
    )
...
def _place_buy_and_verify(
    ...
    logger.info(
        "[BUY] Quantité calculée: %s %s (~%.2f %s)",
        qty_str,
        ctx.coin_symbol,
        quote_amount,
        ctx.quote_currency,
    ...
    client_order_id = _new_kraken_client_order_id()

    # WAL B-05: enregistrer l'intent avant l'appel exchange
    wal_write(OP_BUY_INTENT, ctx.real_trading_pair,
              qty_str=qty_str, entry_price=entry_price, atr_value=atr_value,
              sl_price=_levels.stop_loss, client_order_id=client_order_id)

    try:
        buy_order = deps.market_buy_fn(
            symbol=ctx.real_trading_pair,
            quoteOrderQty=quote_amount,
            newClientOrderId=client_order_id,
        )
    except Exception as buy_err:
        buy_order = _lookup_buy_by_client_order_id(ctx, deps, client_order_id)
        if not buy_order:
            _activate_emergency_halt(
                ctx,
                deps,
                "Etat ordre BUY inconnu apres exception AddOrder",
                details=f"client_order_id={client_order_id}, error={buy_err}",
            )
            return None
        status = str(buy_order.get('status', 'UNKNOWN'))
        if status != 'FILLED':
            if status not in {'CANCELED', 'REJECTED', 'EXPIRED'}:
                _activate_emergency_halt(
                    ctx,
                    deps,
                    "Ordre BUY retrouve mais non final apres exception AddOrder",
                    details=f"client_order_id={client_order_id}, status={status}, error={buy_err}",
                )
            return None
        ...
        try:
            deps.send_alert_fn(
                ...
                body_main=(
                    ...
                    f"Paire : {ctx.real_trading_pair}\n"
                    f"Quantité : {qty_str}\n"
                    f"Montant : {quote_amount:.2f} {ctx.quote_currency}\n"
                    f"Statut : {_status}\n"
                    f"Prix courant : {ctx.current_price:.4f} {ctx.quote_currency}\n\n"
                    f"Action : le bot réessaiera au prochain cycle."
                ),
                client=deps.client,
            )
    ...
    logger.info(
        "[BUY] Capital réellement dépensé : %.2f %s (disponible: %.2f)",
        _actual_spent,
        ctx.quote_currency,
        usdc_for_buy,
    )
    ...
    # === EMAIL ACHAT RÉUSSI ===
    _sl_at_entry = _levels.stop_loss
    _sl_str = f"{_sl_at_entry:.4f} {ctx.quote_currency}" if _sl_at_entry else "N/A"
    ...
    _partial_qty = float(actual_qty_str) * float(getattr(deps.config, 'partial_pct_1', 0.5))
    _can_execute_partial_safely = cast(Any, can_execute_partial_safely)
    can_partial, partial_reason = _can_execute_partial_safely(
        coin_balance=float(actual_qty_str),
    ...
    if not can_partial:
        logger.info("[BUY] Partials desactives pour %s: %s", ctx.real_trading_pair, partial_reason)
    ps.update({
        'entry_price': entry_price,
        'atr_at_entry': atr_value,
        'stop_loss_at_entry': _levels.stop_loss,
        'stop_loss': _levels.stop_loss,
        'trailing_activation_price_at_entry': _levels.trailing_activation,
        'atr_stop_multiplier_at_entry': _levels.atr_stop_multiplier,
        ...
        'entry_strategy': (
            dict(ps['active_strategy'])
            if isinstance(ps.get('active_strategy'), dict)
            else None
        ),
    })
...
def _rollback_on_sl_failure(
    ...
    ps = ctx.pair_state
    try:
        rollback_order = deps.market_sell_fn(symbol=ctx.real_trading_pair, quantity=actual_qty_str)
        if not rollback_order or rollback_order.get('status') != 'FILLED':
            _status = rollback_order.get('status', 'UNKNOWN') if rollback_order else 'None'
            _activate_emergency_halt(
                ctx,
                deps,
                "Rollback apres echec SL non FILLED",
                details=f"status={_status}, quantity={actual_qty_str}, sl_error={reason_err}",
            )
            return
        ps.update({
            'entry_price': None,
        ...
        _write_reconcile_required_marker(
            deps,
            f"Double echec SL+rollback pour {ctx.real_trading_pair}: {_rollback_err}",
        )
        deps.save_fn(force=True)
        try:
            deps.send_alert_fn(
                ...
                body_main=(
                    ...
                    f"ACTION REQUISE: Vérifier la position manuellement sur {_exchange_label(deps)} "
...
def _execute_buy(ctx: '_TradeCtx', deps: '_TradingDeps') -> None:
    ...
    _ps_buy_min  = ctx.best_params.get('stoch_buy_min', ps.get('stoch_buy_min'))
    _ps_buy_max  = ctx.best_params.get('stoch_buy_max', ps.get('stoch_buy_max'))
    ...
    usdc_for_buy = _resolve_buy_capital(ctx, deps, _since_ms)

    if buy_condition and usdc_for_buy <= 0:
        logger.warning(
            "[BUY BLOCKED CAPITAL] %s signal BUY valide mais achat bloque: "
            "aucune vente historique de reference, budget=%.2f %s",
            ctx.backtest_pair,
            usdc_for_buy,
            ctx.quote_currency,
        )
        logger.warning(
            "[BUY BLOCKED CAPITAL] %s aucune vente trouvee dans l'historique; "
            "regle stricte latest SELL appliquee.",
            ctx.backtest_pair,
        )
        _alert_buy_blocked_no_reference_sell(ctx, deps)

    elif usdc_for_buy > 0:
        logger.info("[BUY] Capital disponible pour achat : %.2f %s", usdc_for_buy, ctx.quote_currency)
    ...
    # MULTI-PAIR: sérialiser uniquement l'allocation de la devise de cotation
    # (fetch → sizing → buy). Dès que le buy est FILLED, le cash quote est dépensé
    # côté exchange → on relâche
    ...
    try:
        ...
        # Refresh quote balance sous le lock (garde l'ancienne si le fetch échoue ou retourne 0)
        try:
            _fresh_acc = deps.client.get_account()
            _, _fresh_quote, _, _ = _get_coin_balance(_fresh_acc, ctx.quote_currency)
            if _fresh_quote > 0:
                ctx.usdc_balance = _fresh_quote
    ...
    if actual_qty_str is not None and quantity_rounded is not None:
        try:
            ...
            if not _place_sl_after_buy(ctx, deps, actual_qty_str):
                return
            ...
            # Rafraîchir le solde quote après achat
            account_info = deps.client.get_account()
            _, ctx.usdc_balance, _, _ = _get_coin_balance(account_info, ctx.quote_currency)
        except Exception as e:
