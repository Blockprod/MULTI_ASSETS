    ...
    *,
    ...
    try:
        ...
        for _, candle in recent_candles.iterrows():
            candle_price = float(cast(Any, candle['low']))
...
def get_last_sell_trade_usdc(
    ...
    try:
        ...
        if not trades:
            logger.info("Aucun trade trouve pour cette paire.")
            return None, None, None

        aggregated_sells: Dict[str, float] = {}
        aggregated_commissions: Dict[str, float] = {}
        commission_assets: Dict[str, str] = {}
        ...
        for trade in trades:
            ...
            if not trade['isBuyer'] and float(trade['quoteQty']) > 0:
                ...
                trade_time = int(float(trade.get('time', 0) or 0))
                if order_id not in aggregated_sells:
                    ...
                    order_times[order_id] = trade_time
                aggregated_sells[order_id] += quote_qty
                aggregated_commissions[order_id] += commission
                order_times[order_id] = max(order_times[order_id], trade_time)

        if aggregated_sells:
            latest_order_id = max(aggregated_sells, key=lambda oid: order_times.get(oid, 0))
            total_amount = aggregated_sells[latest_order_id]
            total_fee = aggregated_commissions[latest_order_id]
            fee_asset = commission_assets[latest_order_id]
            logger.debug(
                f"Derniere vente : Order ID={latest_order_id}, "
