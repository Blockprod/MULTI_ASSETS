...
class Config:
    ...
    kraken_ws_url: str = "wss://ws-auth.kraken.com/v2"
    ...
    allow_baseline_live: bool = False
    ...
    oos_sharpe_min: float = 0.15     # Kraken balanced: edge OOS positif requis, sans exiger un Sharpe crypto trop eleve
    oos_win_rate_min: float = 30.0   # P1-THRESH: seuil OOS Win Rate minimum (%)
    oos_decay_min: float = 0.10      # Kraken balanced: anti-overfit garde, mais moins strict que 0.20 sur regimes crypto courts
    oos_strict_mode: bool = True     # C1: True = blocage strict (défaut prod). False = warn-only via OOS_STRICT_MODE=false
    oos_min_trades: int = 6          # Kraken balanced: minimum 1h; 4h=7 et 1d=2 restent adaptes dans walk_forward
    oos_min_folds: int = 3           # Kraken balanced: au moins 3 fenetres OOS independantes avant entree LIVE
    ...
    strict_strategy_parity: bool = True  # Shared Python strategy policy for backtest/live
    backtest_engine_mode: str = 'cython_required'
    wf_optuna_timeout_seconds: int = 1800
    wf_optuna_progress_trials: int = 10
    wf_optuna_progress_seconds: int = 60
    ...
    def from_env(cls) -> 'Config':
        ...
        broker = os.getenv('BROKER', 'BINANCE').upper()
        if broker not in {'BINANCE', 'KRAKEN'}:
            raise ValueError("BROKER invalide: attendu BINANCE ou KRAKEN")
        credential_vars = (
            {'api_key': 'KRAKEN_API_KEY', 'secret_key': 'KRAKEN_SECRET_KEY'}
            if broker == 'KRAKEN'
            else {'api_key': 'BINANCE_API_KEY', 'secret_key': 'BINANCE_SECRET_KEY'}
        )
        required_vars = {
            **credential_vars,
        ...
        # Optionnels
        config_data['kraken_api_url'] = os.getenv('KRAKEN_API_URL', 'https://api.kraken.com')
        config_data['kraken_ws_url'] = os.getenv('KRAKEN_WS_URL', 'wss://ws-auth.kraken.com/v2')
        config_data['enable_native_trailing'] = (
            os.getenv('ENABLE_NATIVE_TRAILING', 'false').lower()
            in ('true', '1', 'yes'))
        config_data['strict_pair_validation'] = (
            os.getenv('STRICT_PAIR_VALIDATION', 'true').lower()
            in ('true', '1', 'yes'))
        ...
        _default_cache = 'cache_kraken' if broker == 'KRAKEN' else 'cache'
        _cache_env = os.getenv('CACHE_DIR', _default_cache)
        ...
        _default_state_file = 'bot_state_kraken.json' if broker == 'KRAKEN' else 'bot_state.json'
        config_data['state_file'] = os.getenv('STATE_FILE', _default_state_file)  # C-17
        ...
        config_data['sizing_mode'] = os.getenv('SIZING_MODE', 'risk')
        config_data['allow_baseline_live'] = (
            os.getenv('ALLOW_BASELINE_LIVE', 'false').lower()
            in ('true', '1', 'yes'))
        ...
        config_data['oos_min_trades'] = int(
            os.getenv('OOS_MIN_TRADES', '6'))           # Kraken balanced min trades 1h
        config_data['oos_min_folds'] = int(os.getenv('OOS_MIN_FOLDS', '3'))
        ...
        config_data['strict_strategy_parity'] = (
            os.getenv('STRICT_STRATEGY_PARITY', 'true').lower()
            in ('true', '1', 'yes'))
        config_data['backtest_engine_mode'] = os.getenv('BACKTEST_ENGINE_MODE', 'cython_required')
        config_data['wf_optuna_timeout_seconds'] = int(
            os.getenv('WF_OPTUNA_TIMEOUT_SECONDS', '1800'))
        config_data['wf_optuna_progress_trials'] = int(
            os.getenv('WF_OPTUNA_PROGRESS_TRIALS', '10'))
        config_data['wf_optuna_progress_seconds'] = int(
            os.getenv('WF_OPTUNA_PROGRESS_SECONDS', '60'))
    ...
    def _validate(self) -> None:
        ...
        if (
            getattr(self, 'broker', '').upper() == 'KRAKEN'
            and self.bot_mode == 'LIVE'
            and self.sizing_mode == 'baseline'
            and not self.allow_baseline_live
        ):
            errors.append(
...
def extract_coin_from_pair(real_trading_pair: str) -> Tuple[str, str]:
    ...
    quote_currencies = ["USDC", "USDT", "BUSD", "USD", "EUR", "GBP", "CAD", "AUD"]
